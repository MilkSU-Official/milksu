...
test('tray and dock share companion actions; the application menu is not a companion menu', () => {
  ...
  assert.deepEqual(labels.filter(Boolean), ['对话', '隐藏看板娘', '打开主窗口', '看板娘设置', '退出'])
  assert.ok(dock.menu)
  assert.deepEqual(dock.menu.template.map(item => item.label).filter(Boolean), ['对话', '隐藏看板娘', '打开主窗口', '看板娘设置'])
  const statusLabels = shell.status().menu.map(item => item.label)
  assert.deepEqual(statusLabels, ['对话', '隐藏看板娘', '打开主窗口', '看板娘设置', '退出'])
  const appLabels = JSON.stringify(menuApi.lastApplication || {})
  assert.equal(appLabels.includes('看板娘'), false)
...
test('ShowCompanionChatWindow replaces the pet with the phone, not a second window', () => {
  const { shell, created } = createShell()
  shell.createFloat()
  assert.equal(created[0].options.title, '看板娘')
