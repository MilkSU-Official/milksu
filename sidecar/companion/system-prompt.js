...
export function companionSystemPrompt(uiLocale, replyStyle = "markdown") {
  ...
  if (chineseUiLocale(uiLocale)) {
    const lines = [
      ...
      "调研、摸底、查因优先 companion_dispatch 交给合适对话。Working 的结果只在用户正在问这件事时再讨论。",
  ...
  const lines = [
    ...
    "For research, reconnaissance, and root-cause work, prefer companion_dispatch into a suitable conversation. Discuss Working results only when the user is asking about that work.",
