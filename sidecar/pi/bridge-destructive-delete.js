...
/**
 * True when the command itself writes that path (`> x.sh`, `tee x.sh`, `cp a x.sh`). A script a
 * command writes does not exist yet when the decision is made, so its contents cannot be read -
 * and it must not be mistaken for "this command deletes nothing".
 */
export function writesPath(command, file) {
  const target = String(file ?? "").trim();
  if (!target) return false;
  const text = String(command ?? "");
  if (!/(?:^|[\s;&|(])(?:\d?>>?|tee\b|cp\b|mv\b|install\b)/.test(text)) return false;
  const wanted = resolve(target);
  for (const raw of text.split(/[\s;&|()<>]+/)) {
    const word = raw.replace(/^['"]+|['"]+$/g, "").trim();
    if (word && resolve(word) === wanted) return true;
  }
  return false;
}
