...
| Layer | Owns | Use |
| --- | --- | --- |
| Materials | tokens, color, type, motion | shadcn structure, Cursor Light / Cursor Dark surfaces. Page and sidebar fills stay high-alpha (about 70–90%) so macOS `under-window` vibrancy and Windows acrylic can show a sliver of wallpaper; Linux stays opaque. Dark page `#181818` / sidebar `#141414`; light page `#fcfcfc` / sidebar `#f3f3f3`. Overlay Dialog / Popover / Sheet / DropdownMenu stay fully opaque `#141414` / `#f3f3f3` — no `backdrop-filter` on overlays (Windows Chromium often skips frost and a thin fill punches a hole). No OLED black (`#09090b`), no zinc-950, no stark `#ffffff` page, no wash stack, no carbon texture, no page-level cyan. `--primary` is high-contrast ink (`#f0f0f0` on dark, `#141414` on light) for primary buttons and ink fills — do not recolor it as the user accent. User accent is `--emphasis` / `--emphasis-foreground` (Settings → 通用 presets: default ink, blue, violet, teal, amber, rose; live). Use `--emphasis` sparsely: Switch / Checkbox checked, focus `--ring` when a colored preset is active, progress bars, and small selection checks. Do not wash page or sidebar fills with emphasis; keep overlays grayscale. Sidebar update action uses fixed `--update` (Cursor-like blue: `#5b9fff` dark / `#2563eb` light), not `--primary` or `--emphasis`. shadcn `--accent` stays the hover gray surface, not brand color. Radius 8px (`rounded-md`). Not Mica as the product look. Fonts default to Inter Variable + Noto Sans SC Variable. Settings → 通用 can change interface font, conversation font, and their sizes (presets, live). Motion about 200ms. |
