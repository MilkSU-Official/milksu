...
- The last receipted three-platform GitHub Release is `v26.904.1` at `6e9371d`. Write both that
  baseline and the current development version line when HEAD is later. Keep `v26.827.1` as a
...
- A version bump, empty tag, local dirty package or later `main` commits on the same version number
  are still not a new ship. `26.904.1` is a receipted GitHub Release; commits after `6e9371d` are not.
