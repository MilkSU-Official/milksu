...
- The last receipted three-platform GitHub Release is `v26.827.1` at `37932ce`. Write both that
  baseline and the current development version line when HEAD is later. Keep `v26.825.1` as a
  previous downloadable release, not Latest.
...
- A version bump, empty tag, local dirty package or later `main` commits on the same version number
  are still not a new ship. `26.827.1` is a receipted GitHub Release; commits after `37932ce` are not.
