...
| Layer | Owns | Use |
| --- | --- | --- |
| Materials | tokens, color, type, motion | shadcn structure, Cursor Light / Cursor Dark surfaces. Page and sidebar fills stay high-alpha (about 70–90%) so macOS `under-window` vibrancy and Windows acrylic can show a sliver of wallpaper; Linux stays opaque. Dark page `#181818` / sidebar `#141414`; light page `#fcfcfc` / sidebar `#f3f3f3`. Overlay Dialog / Popover / Sheet / DropdownMenu stay fully opaque `#141414` / `#f3f3f3` — no `backdrop-filter` on overlays (Windows Chromium often skips frost and a thin fill punches a hole). No OLED black (`#09090b`), no zinc-950, no stark `#ffffff` page, no wash stack, no carbon texture, no page-level cyan. Factory `--primary` is high-contrast ink (`#f0f0f0` on dark, `#141414` on light). Settings → 通用「强调色」presets (default ink, blue, violet, teal, amber, rose; live) drive `--emphasis` and, for colored presets, also remaps `--primary` / `--primary-foreground` / focus `--ring` so send and brand buttons, selected filter chips, Switch / Checkbox, progress bars, sidebar / tools selected inset strip, and `text-primary` checks follow the accent. Default preset clears those remaps and keeps ink. Do not wash page or sidebar fills with color; keep overlays grayscale. Sidebar update action stays fixed `--update` (Cursor-like blue: `#5b9fff` dark / `#2563eb` light), not the user accent. shadcn `--accent` stays the hover gray surface, not brand color. Radius 8px (`rounded-md`). Not Mica as the product look. Fonts default to Inter Variable + Noto Sans SC Variable. Settings → 通用 can change interface font, conversation font, and their sizes (presets, live). Motion about 200ms. |
| Shell | sidebar, topbar, page column | One sidebar + one page column. Collapsed 52px; expanded min 224px, default 264px, drag the right edge. Selected rows are an 8px rounded rectangle. Conversation rows open a right-click context menu (prevent the browser menu): 置顶 / 取消置顶, pin order when already pinned, 重命名, Fork, 复制, 归档, 删除. Archive runs immediately; only permanent delete asks for confirmation. Conversation rows show relative activity age on the right (`1m` / `16h` / `5d`, same helper as the command panel). Hover pin and archive take that slot. Do not put a kebab / three-dot button on the row. Same 8px hit targets and `--hover-2`. Pinned section uses the pin mark, not a folder. Project folders use closed `Folder` when collapsed and `FolderOpen` when expanded. Sidebar search (and Cmd/Ctrl+K) opens one centered opaque command panel: flush search field, filter chips 全部 / 会话 / 设置 / 命令, recent chats with an accent dot + workspace name + relative age, then settings or slash when those filters (or a query) ask for them. Footer shows 打开 / 选择 / 关闭 keys. It is not a second sidebar, not an inline list filter, and not a bordered form dialog with empty document icons. Overlay fill is `--surface-overlay` / `--popover`; no `backdrop-filter`. Footer: version plus a blue `--update` text control 「更新」/Update when an update is available (download progress is percent on that same control — not a gray download icon), then theme and settings icons; both icons stay when collapsed. Workspace avatar menu is only as wide as its items. Settings replaces that same sidebar with the category list — do not keep the workspace sidebar and add a second settings nav. Settings back is a full-width header row (chevron + 设置/Settings), not an icon-only button. `--page-stack-width` 64rem. Window frame is one shell: macOS `hiddenInset` with traffic lights over the sidebar; Windows and Linux hide the native caption and in-window menu, paint a canvas-colored overlay, system buttons top-right. No mac traffic-light hole on Windows or Linux, and no second white title bar. |
| List chrome | filters, History, primary action | shadcn Button / Input / Select / Badge. Filters are a row of `Button` (`outline` / selected `default`, `rounded-md` or `rounded-full`). Catalog tables use canvas or card fill and ordinary caption headers — not monospace uppercase desk heads. Category / difficulty / severity use `Badge`. Conversation list chrome matches the sidebar: hover pin / archive, right-click for the full action menu — no kebab on the row. Command-panel filter chips use the same selected-fill / unselected-text row. |
| Facts | cards, tables, dialogs, status | shadcn Card / Table / Dialog / Alert / Switch. Settings follow Cursor: one `SettingsSection` list, `SettingsRow` is label left and one compact control right. Label and control share `--text-label` (13px); control height is 28px. Persist on change — no page-level Save / Save and verify. Vendor / app marks (editor, model) sit inside the Select option and the closed trigger, not as a sibling icon beside the control. No nested cards, no workbench panes, no stacked ActionCards inside a category. Approvals, credentials, and in-place form errors stay `Alert`. Background and short-lived failures use a toast (`Toaster`, opaque `--popover`). Do not put API keys in toasts. The update-restart dialog is the same opaque Dialog: it asks whether to quit when a turn is running, then one confirm applies the downloaded update and relaunches. |
| Copy | user-visible strings | `t('中文', 'English')`; empty controls stay blank |
| Agent conversation | Coding / CTF / CVE / lab chat | shadcn for chrome. Empty new conversation centers the heading and composer in the column; after the first message the composer stays at the bottom. Product contracts stay: resident composer, busy Queue / Steer, process and tool disclosure, `milksu_ask` rows (last row is 其他 / Other), plan, code blocks, real harness tokens. The composer model chip opens a first-level menu only: Model, Reasoning (when the model has levels), Context size (display only), and Runtime. Do not open any submenu on first click or focus; a row flies the second panel out only on hover (right, or left if it would clip). The flyout stays inside the window: pin to the first panel’s bottom near the composer and shrink with a scrolling list. Do not keep a separate thinking chip on the composer. The composer git chip opens a compact searchable popover: search field, max-height scroll, current checkmark, and Create Branch from the typed name. Do not paint the full local-branch list. Settings default / subagent pickers stay a single searchable popover. Composer image attachments are thumbnails; `@` attaches through the existing file-choose / attach RPC — do not add a workspace indexer. Right rail and bottom terminal are a plain `aside` / Card (`ContextRail` for the resizable right column). |
