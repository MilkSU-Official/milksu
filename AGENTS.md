...
- The last receipted three-platform GitHub Release is `v26.825.1` at `efddfc2`. Write both that
...
- A version bump, empty tag, local dirty package or later `main` commits on the same version number
  are still not a new ship. `26.825.1` is a receipted GitHub Release; commits after `efddfc2` are not.
