...
function clampGeometry() {
  ...
  const maxLeft = Math.max(minLeft, window.innerWidth - width.value - EDGE)
  const maxTop = Math.max(MIN_TOP, window.innerHeight - height.value - EDGE)
  left.value = Math.min(maxLeft, Math.max(minLeft, left.value ?? minLeft))
...
function defaultExpandedPosition() {
  const minLeft = sidebarLeftBound()
  left.value = Math.max(minLeft, window.innerWidth - width.value - EDGE)
...
watch([width, height, left, top], persistGeometry)
...
<template>
  ...
  >
    ...
    <div class="conversation-dock__main">
    ...
    <template v-if="placement !== 'column'">
