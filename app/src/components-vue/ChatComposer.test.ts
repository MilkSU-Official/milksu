...
describe('ChatComposer', () => {
  ...
  it('keeps the two bottom choosers on one hover language and shows full approval labels', async () => {
    ...
    const harness = host.querySelector('[aria-label="选择 Agent 运行时"]')
    expect(harness?.className).toContain('composer-control')
    expect(harness?.className).toContain('composer-harness')
    ...
    expect(composerControlsSource).toContain('.composer-harness {')
    expect(composerControlsSource).toContain('.composer-runtime-pickers')
