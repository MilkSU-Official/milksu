...
describe('Workspace visual contract', () => {
  ...
  it('lets chrome follow the document theme instead of pinning night graphite in day mode', () => {
    ...
    expect(settingsPageSource).not.toContain('settings-focus-row')
    expect(settingsPageSource).not.toContain('model-service-row-primary')
    expect(settingsPageSource).not.toContain('serviceIsActiveDefault')
    expect(appStylesSource).not.toContain('.settings-focus-row')
