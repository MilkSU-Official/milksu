...
import pluginSettingsPanelSource from '../components/PluginSettingsPanel.tsx?raw'
...
describe('plugin theme bridge contract', () => {
  it('passes the resolved core theme through settings into the isolated frame', () => {
    expect(appSource).toContain('const resolvedTheme = useMemo<ResolvedThemeMode>')
    expect(appSource).toContain('resolvedTheme={resolvedTheme}')
    ...
    expect(pluginSettingsPanelSource).toContain('buildPluginFrameDocument(plugin.id, nonce, theme)')
    expect(pluginSettingsPanelSource).toContain('useEffect(() => {\n    syncFrameTheme()')
    expect(pluginSettingsPanelSource).toContain('onLoad={syncFrameTheme}')
  })
