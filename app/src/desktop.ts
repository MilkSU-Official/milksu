...
interface DesktopAppBindings {
  ...
  /** 0..1 danger score from the decision layer; rejects when neither the decision credential nor the session model is reachable. */
  JudgeApprovalRisk(command: string, conversationId: string): Promise<number>
...
export async function invokeCommand<T = unknown>(command: string, args?: CommandArgs): Promise<T> {
  ...
  switch (command) {
      ...
      case 'send_companion_message':
        return app.SendCompanionMessage(
