...
export default function ComposerWorkspaceMenu({
  ...
  useLayoutEffect(() => {
    ...
    if (!rect) return
    setFlyoutLeft(rect.right + FLYOUT_WIDTH + 8 > window.innerWidth)
  }, [sub])
  ...
  function toggleSub(next: WorkspaceMenuSub) {
  ...
  const flyoutClass = cn(
    'absolute top-0 z-10 w-64 rounded-md border border-border bg-popover p-1 text-popover-foreground shadow-md',
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" side="top" sideOffset={8} className="w-60 p-1">
        <div ref={panelRef} className="relative">
          ...
          <button
            type="button"
            className={cn(subTriggerClass(false), !hasSelectedWorkspace && 'opacity-55')}
          ...
          {sub === 'new' ? (
            <div className={cn(flyoutClass, 'p-2')}>
              <Input
                value={newName}
                autoFocus
