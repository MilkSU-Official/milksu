...
async function buildSidecar(platform) {
  ...
  await Promise.all([
    ...
    copyFile(
      join(repositoryRoot, 'sidecar', 'pi', 'custom-relay-transport.cjs'),
      join(output, 'custom-relay-transport.cjs'),
    ),
    copyFile(
    ...
    copyFile(
      join(repositoryRoot, 'sidecar', 'pi', 'playwright-session-bridge.cjs'),
      join(output, 'playwright-session-bridge.cjs'),
    ),
    copyFile(
  ...
  await Promise.all([
    ...
    chmod(join(output, 'custom-relay-transport.cjs'), 0o644),
    chmod(join(output, 'playwright-session-bridge.cjs'), 0o644),
...
async function installSidecar(platform, binaryPath) {
  ...
  const distributableFiles = [
    ...
    'pi-subagents-spawn.cjs',
    'current-provider-runtime.cjs',
    'custom-relay-transport.cjs',
    'known-context-window.cjs',
    'playwright-session-bridge.cjs',
    'plugin-worker.mjs',
    'deny-loader.mjs',
    'pi-subagent-cli.cjs',
    'cua-driver',
