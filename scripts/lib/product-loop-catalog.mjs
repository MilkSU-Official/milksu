...
export const CASES = {
  ...
  'intent-status': item('intent-status', '追问不另派', 'intent', true),
  'intent-done': item('intent-done', '做完她通知', 'intent', true),
  'intent-notices-merge': item('intent-notices-merge', '两条通知都在', 'intent', true),
  'intent-approval': item('intent-approval', '待批她通知', 'intent', true),
  'intent-error': item('intent-error', '真报错她通知', 'intent', true),
  'intent-tool-continues': item('intent-tool-continues', '工具失败后还盯着', 'intent', true),
  'intent-stall': item('intent-stall', '卡住才问说不说', 'intent', true),
  'intent-memory': item('intent-memory', '记忆留或丢', 'intent', true),
  'intent-other-silent': item('intent-other-silent', '没派的会话不说', 'intent', true),
  'intent-notice-waits': item('intent-notice-waits', '开口时不串话', 'intent', true),
  'intent-fallback-record': item('intent-fallback-record', '没接上标明主模型', 'intent', true),
...
export const MODULES = {
  ...
  coding: {
    ...
    needsCredential: true,
    isolated: true,
    detail: 'Pi 与 DSH 日常开发：写文件、后台子代理、改文件、跑命令、插话、排队、停止、整理上下文、选择卡、计划、目标、并行，以及会话壳、输入栏和一条直接写下的用户记忆。',
    cases: [
