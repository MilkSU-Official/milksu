...
| Archify / LSP / Goal / Background | **Mixed** | Archify 已在真实打包 App 一键生成固定产物、showcase 9/9 并右侧预览；固定 `pi-goal`、后台任务、LSP 与 CTF 隔离 Smoke 已有。TypeScript/Vue Server 已随 Sidecar 打包并完成原生诊断；`gopls` 与 `lsp_fix` 审批仍未完成；当前固定清单不再包含 `pi-retry`。 |
...
3. Archify、LSP、Goal、后台任务与 MCP 的真实状态逐项披露；Archify、审批、后台任务、TypeScript/Vue LSP 与资源隔离已验，Go LSP 和写修复缺口不能被加载 Smoke 冒充。
...
| LSP 仍是分语言完成 | TypeScript/Vue 已固定打包并在真实 App 返回确定性诊断；Go 缺审核过的 `gopls`，`lsp_fix` 仍被阻止 | TS/Vue 可演示，Go 项目与自动修复会暴露缺口 | 打包固定 `gopls`，为 `lsp_fix` 接独立审批与 Diff；重试继续依赖 Pi/Provider，不恢复已移除的临时自研循环。 |
