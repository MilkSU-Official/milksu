...
| Coding Plan / Go 与权限策略 | **Implemented / Partial overall** | Codex 风格三档菜单、Project Auto 常规开发 Shell/Git/网络、显式 Full Access 和 Ask 单次工具审批已落地并有交付门禁；右侧终端页已在原生 App 真实运行短命令并启停带 PID/端口/日志的服务；仍缺交互式 PTY 以及 Browser/Computer Use 授权。 |
