...
| `milksu_workspace` | 类型化产品 UI 工具：列出/聚焦/关闭内置浏览器标签，列出/预览产物，打开环境、变更、终端和后台任务，并整理对话、实验室、CVE、CTF 记录。看板娘调用同一个 Go 处理函数；浏览器、终端、产物和后台任务要带目标对话 id。永久删除先确认。不改设置、凭据、审批档，不附着用户 Chrome | 同一套产品 UI 工具 | MilkSU first-party Extension + Desktop RPC |
