...
> 文档状态：Current
>
> 事实审计：2026-08-18；正式发行基线为 `v26.818.2 / ea7d2ff`。文档收口提交不移动该 tag。
>
...
| 内置浏览器 | **Verified packaged tasks; multi-tab in 26.818.2** | 产品 UI 只显示“浏览器”。每次 Coding 会话使用独立 `session.fromPath`，默认拒绝页面权限。`26.817.1` 起已有打包任务：Grok 只用浏览器完成顺序点击、表单提交和公开文档调研，右栏折叠后继续并保留同一页面终态。`26.818.2` 安装包里普通 Coding Go 或打开右栏仍会自动就绪。当前代码改为：隔离浏览器只在用户打开右栏或模型调用类型化 `milksu_workspace` 浏览器动作时启动；普通 Go 问候不再 `EnsureCodingBrowser`。标签栏 `+` 在启动前可见；每个标签是独立 `WebContentsView`，切换换页并更新地址。`ScopedCDPProxy` 仍只公布当前一个 Target。 |
...
`v26.818.2 / ea7d2ff9c44aad3590b53fb71222269ab2c34b34`。仓库版本号是 `26.818.2`。
...
prerelease 只提供 DMG、EXE 与 DEB，没有 OTA ZIP。纯文档提交不改变 `v26.818.2` 的 source commit。
