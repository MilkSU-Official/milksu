...
| 6 Agent 对话 | Coding / CTF / CVE / 实验室共用的对话区 | Beautiful UI 是 Coding Agent 的设计语言。`[data-agent-conversation]`：思考、工具行、审批、Prompt 岛、代码块；除代码块外不加对话卡片 |

行为仍走 Felinic。第 1–4 层（壳、列表、设置、档案）仍走 ak-ui token。不要把 `@yunyoujun/ak-ui` 写进 `app/package.json`。
**对话区的设计语言是 Beautiful UI**（MIT，[beautifului.dev](https://www.beautifului.dev/)）。不要把设置卡片或列表指挥面套进 Agent 循环，也不要把对话区的工具行/思考条套回设置页。不要把 Beautiful UI 的 React 运行时、`globals.css` 或付费 `@central-icons-react` 写进产品链；只沿用交互骨架，落在 MilkSU token 上。
...
- 材质：冷中性纸面。助手正文、工具行、审批、压缩条是排版，不加描边圆角底。用户消息是圆角气泡（半径 16）。代码块是深色圆角块（半径 8）。Prompt 岛仍是输入面（半径 16）。强调色可以是一条蓝（思考点、发送），不再用战术切角气泡或 YOU / MILKSU 字标。
...
- 思考：Beautiful UI 式可展开思考条，单独一行，不包进回复卡片。进行中用 Drive 像素格 + 「正在思考」+ demo 时长（`0.1s` / `1m 2.3s`）。结束后是「想了」和同一格式。时长不进 Composer。正文只来自 Pi 的 `thinking` 块。
- 工具：一行文字（`Read` / `Edit +N -N` / `bash` + 路径 + 时长），没有芯片卡片。点开才看输入/结果。工具或审批跑完后默认隐藏；用户点开过的条目保持可见，不要再刷回隐藏。
- 审批：同一列的标题 + 按钮，不加 HITL 卡片壳。按钮仍是拒绝 / 允许这一次 / 本对话始终允许。结束后同样隐藏，除非用户已经展开。
- 回答：同一列里的正文，不另缩一截。来源是下划线链接，只接线真实 `https`。助手回复可复制，并可从该条分叉到新对话（Pi `fork` / `createBranchedSession`）。
- 用户消息：右缘圆角气泡。可复制、可编辑；操作是线条小图标，不是文字按钮。编辑时仍在同一个圆角框里改，不要变成只有底边的输入。编辑后发出即从该条重新开始（Pi `navigateTree`，丢掉后面的回合）。
- 代码块：深色圆角块，带复制。
