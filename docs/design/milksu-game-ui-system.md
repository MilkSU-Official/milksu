...
MilkSU 使用一套任务型游戏界面语言：像游戏任务与指挥台一样明确焦点、状态和下一步，同时保留
开发工具与管理后台需要的信息密度。游戏感来自层级、几何、反馈和任务连续性，不来自积分、连续
打卡、装饰性图表或霓虹噪声。
...
6. 深浅主题共享尺寸、结构、状态和层级；日间模式使用纯白与冷灰蓝，不使用米黄或暖灰底。
...
- **App Shell**：深色海军蓝或纯白主画布；固定 rail；一像素冷色分隔线。
- **Module Header**：模块图标、清晰大标题、右侧操作；筛选和收藏夹独立成第二层。
- **Focus Panel**：绿色左缘或角切标记；只给当前任务、选中记录或审阅对象。
