...
用户和任务事实。酸绿色表示当前目标、选中、确认和主要操作；蓝色只保留给执行阶段与诊断信息；斜切、档案叠页和
印刷微标建立游戏感，不依靠积分、连续打卡、装饰性图表或霓虹噪声。

本轮实现的唯一视觉源是用户确认的[方案 2](game-ui/tactical-archive-approved.png)。被它取代的旧六张
独立页面批准稿已从当前工作树删除，需要考古时使用 Git 历史；它们不再覆盖方案 2 的材质、颜色、
组件几何与首屏层级。
...
个人资料页不再保留独立概念原型作为视觉来源；它应从当前产品代码、共享组件语言和实际运行验收
判断，避免已退役的仪表盘或能力画像范式重新进入设计。

## 不变量

1. Desktop 主导航在同一窗口宽度下保持固定，不随模块伸缩。宽屏显示图标和文字；窄屏只显示图标。
2. 左上角永远是当前用户的圆形头像，不用品牌 Logo 冒充用户。
3. 酸绿色表示身份、选中、已确认和主要操作；蓝色只表示链接、执行阶段和诊断信息。
...
- **Primary Action**：酸绿色底配深色文字；页面同时最多一个主操作组。
...
- **隐藏控制面**：会话抽屉、Coding 右栏、浏览器 chrome、变更/架构/历史面板与所有菜单、选择器、对话框共用深色战术控制面；页面不再各自复制一套浮层背景和文字角色。
- **窄屏**：主内容按容器宽度重排；任务标题与目标允许换行，阶段轨从四列降到两列/单列；右栏空间不足时覆盖而不继续挤压主区，领域档案缩小页边并隐藏无助于阅读的叠纸装饰。
