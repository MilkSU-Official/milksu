...
- **Coding 实时网页查证已接入 Pi Extension**：MilkSU 直接装载 Pi PR `#3080`、revision `53e430c` 的 `web_search` / `web_fetch` 扩展，不另建搜索路由、关键词判断或搜索状态机。扩展零账户配置地通过 Jina Reader 搜索 DuckDuckGo Lite 并读取目标页面；真实查询“Grok 4.5 是否支持图片输入”已找到并读取 xAI 官方模型文档。该 PR 当前未合并，因此代码以固定 revision 进入发行依赖；自动化与真实联网工具回执已通过，Stable GUI 仍待用户验收。
