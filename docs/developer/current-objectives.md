...
- 账户 TokenFlux 与本机 Provider 共用可调用目录；保存的模型 id 跟目录真实后缀走（例如目录只有 `gemini-3.8-flash-tiered` 时不再请求无后缀的 `gemini-3.8-flash`）。设置里添加模型提供商：目录提供商只填 API 密钥，自定义模型 API 再填 Provider ID、显示名称、API 地址、协议和模型。协议以用户选择为准，没选时自定义中转站按用户填写的端点选 Pi 协议：`api.anthropic.com` 或路径含 `anthropic` 走 Anthropic Messages（思考块；`/v1` 后缀会去掉，避免请求打到 `/v1/v1/messages`），`generativelanguage.googleapis.com` 或路径含 `gemini` / `google` 走 Gemini，其余走 OpenAI chat completions。思考字段按 models.dev 的 `reasoning` / `interleaved.reasoning_content` 对齐到 Pi 已有格式：OpenRouter 统一用它自己的 reasoning 字段；DeepSeek、Claude（OpenAI 兼容路径）、Kimi、MiniMax、豆包回放 `reasoning_content`；Qwen 用 `enable_thinking`；GLM 用智谱格式；GPT 用 `reasoning_effort`。官方 Grok 仍不发 `reasoning_effort`。附件原图进当前回合。网页查证复用 Pi `web_search` / `web_fetch`。
