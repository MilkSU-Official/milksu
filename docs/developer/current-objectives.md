...
- 窄逆向出厂覆盖对齐 Security 终裁（PR #55，未合并）：babysitter `ghidra-ida-re` 不出厂，IDA 只保留 `milksu-ida-pro`。`ghidra-rpc` 内置 overlay 默认关，检测 Ghidra≥11 / Java≥17 / `GHIDRA_INSTALL_DIR` 才为 ready；钉 `cellebrite-labs/ghidra-rpc` main `1743305487b1de754fb750486dd468ea4d3c4141`，短 when-to-use，NOTICE 写明上游无 LICENSE 文件。JADX 只 vendor `mukul975` 的 `skills/reverse-engineering-android-malware-with-jadx` @ `v1.3.0` / `101ca0bd887a295e39cc20a100efa571937ca969`，默认关，样本限 lab / InjuredAndroid，输出清洗，不用 Computer Use。Coding / Lab 会签仍开。跟踪见 [窄逆向 Skill / MCP 出厂覆盖](narrow-re-skill-overlays.md)。不是 Kali 商店，也不扩七言全表。
...
| P1 | 安全工具真实任务 | IDA/idalib 与 capa 已有设置、准备和健康检查；用受控本地样本留下真实任务回执。就绪工具接到实验室作业，窄工具也可进 CVE 复现；不需要先开一次“是否投影”的会。不把 HexStrike 整包 MCP 做成产品页或 Kali 应用商店。CodeQL、Burp、Shannon 仍逐项接入。窄逆向：babysitter `ghidra-ida-re` 不出厂；`ghidra-rpc` 与 JADX Skill 已按 Security CONDITIONAL PASS 落地（默认关，ready+启用才进名录），Coding / Lab 会签仍开；见 [窄逆向 Skill / MCP 出厂覆盖](narrow-re-skill-overlays.md)。 |
