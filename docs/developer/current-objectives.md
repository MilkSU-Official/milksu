...
- **Coding 实时网页查证已接入 Pi Extension**：MilkSU 直接装载 Pi PR `#3080`、revision `53e430c` 的 `web_search` / `web_fetch` 扩展，不另建搜索路由、关键词判断或搜索状态机。扩展零账户配置地通过 Jina Reader 搜索 DuckDuckGo Lite 并读取目标页面；真实查询“Grok 4.5 是否支持图片输入”已找到并读取 xAI 官方模型文档。该 PR 当前未合并，因此代码以固定 revision 进入发行依赖；自动化、真实联网工具回执和 Stable GUI 用户验收均已通过。
...
- 用户可见产物固定写入 `~/Documents/MilkSU/{Coding,CTF,CVE}`，设置页可直接打开。普通用户文件通过 Composer 的附件入口进入受管附件区，选择、粘贴和拖放统一为可排序、预览、移除及恢复的 Pi 附件描述；普通 Coding 与 CTF 的通用文件和 Shell 均使用 Pi 内置工具及当前系统用户权限，MilkSU 不再维护第二套 workspace-only 工具。CTF 仍保留题目域工具、精确站点能力、Judge 和证据投影；模型因输出长度结束时通过 Pi `agent_end` / `followUp` 原生扩展点继续，而不把半句当成正常完成。Provider Key 只走模型设置和隔离的模型进程，不进入 Bash、后台任务、工具输出或模型上下文。无项目 Coding 会话统一显示为“无项目任务”，实际临时工作区转入 `Application Support/<bundle-id>/agent-workspaces`，不再在 Documents 生成“新编码任务-哈希”目录；CVE 等用户可见产物仍留在 Documents。Runtime、事件库、凭据、Obelisk、浏览器 Profile 和内部恢复数据继续留在平台用户配置目录，两类目录不混用。递归删除用户 Home、文件系统根、当前会话 cwd 或大型目录时，Pi 工具前置钩子在任何审批档位都要求展示规范化目标和影响并再次确认。
