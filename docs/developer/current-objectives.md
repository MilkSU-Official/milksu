...
当前执行阶段先使用
[当前目标覆盖台账](./objective-coverage-ledger.md) 对本文件全部目标做广度优先盘点：

1. 先把大项拆成可独立判定的细项，让所有一级目标完成一轮证据核对；
2. 每发现一个问题先登记稳定 ID、证据、分值和缺口，不立即修改产品代码；
3. 初轮盘点完成前，不因单个问题继续向下深挖或顺手修复；
4. 用户与主 Agent 共同评估台账后，再明确选择一批问题进入修复；
5. 每批修复结束后回到台账更新同一口径，避免靠大项印象或移动完成线提高百分比。
...
B1–B5 的真实自举门槛通过后，Coding Agent 的下一优先级不是继续增加低频开发工具，而是
补齐用户实际高频使用、目前替代效果不足的通用能力：
...
高频替代能力的验收以实际任务成功率为准：同时保留 MilkSU 自举任务和其他授权项目任务，
记录完成结果、人工接管次数、恢复、耗时、模型/工具成本和失败分类。安装了 Skill、出现了
按钮或通过单次 fixture 都不能单独证明已经替代。
