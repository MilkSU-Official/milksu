...
> 文档状态：当前唯一目标契约
>
> 执行状态：产品开发已暂停；等待用户与主 Agent 选定首批有界开发范围
>
> 生效日期：2026-08-03
>
...
2. **P1 · 成熟能力集成。** ImageGen、Browser、前端设计/视觉回归 Skill、Project MCP 和
   ...
   Harness。
3. **P1 · CTF 广度验收。** 六个赛道作为独立验收线运行；允许不同 subagent 分赛道执行，
   主 Agent 负责授权边界、证据审阅、问题去重和最终汇总。发现问题先登记，不在单题中无限
   深挖修复。
4. **P2 · Memory 用户体验。** 先保证推荐理由可追溯、Memory 与能力画像对用户可理解、
   删除或归档后的结果一致；36 条完整校准矩阵和精细统计后置。
5. **持续约束。** 架构不做独立清债冲刺，但相关纵切不得继续给热点文件增加新职责；测试、
   Judge 回执、Checkpoint 和必要 ADR 随任务保留。
6. **后期。** 完整“请求批准”组合矩阵、NYU Outcome Bench、pre-release 破坏性收口、
...
目标处于暂停状态时，只允许盘点、讨论和维护目标文档，不自动进入产品实现。恢复开发时，
必须同时明确首批小任务、允许修改范围、禁止顺带修改范围和验收方式，并让工作区持久目标
继续指向本文件及该批次；不得恢复已经被本节取代的旧顺序。
