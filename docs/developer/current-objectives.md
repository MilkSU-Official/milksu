...
1. **P0 · Obelisk 形态 Session Index / Agent 长期记忆。** 当前先把跨 Coding、CTF、CVE 的
   ...
   “相关历史 → 引用到复盘”的显式确认路径；同日真实打包 MilkSU UI 进一步证明用户确认引用
   ...
   prepare → 一次性 token → publish → 复用已有 Draft PR → readback verified 的真实链路；这仍
   不是 WebView 人工点击或新建 PR。剩余优先做真实用户历史
...
2. **P0 · 当前冲刺：完整产品闭环。** Session Index 首轮闭环通过后，继续让 MilkSU 像裸
   ...
   已证明打包 App 进程能为 `CVE-2023-46604` 同步 NVD、FIRST EPSS、OSV、GitHub Advisory、
   CISA KEV 和 Vulhub 练习目录，保留六份来源 snapshot，并匹配到
   `activemq/CVE-2023-46604`。后续不再把 CVE 说成只有 mock 数据或只有单 NVD；CVE
   设置页主同步入口已升级为 NVD、FIRST EPSS、OSV、GitHub Advisory、CISA KEV 和 Vulhub
   六源矩阵，并保留逐源成功/失败状态；同日真实打包 MilkSU UI 已点击该设置入口，
   对 `CVE-2024-3400` 完成 4/4 旧来源同步，并在隔离 App data 下落盘 NVD、FIRST EPSS、
