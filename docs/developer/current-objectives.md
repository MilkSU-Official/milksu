...
7. Lab 纵深闭环保持暂停，不作为当前真实完成条件；CVE 作为一级主菜单必须在当前冲刺中
   形成学习/追踪工作台骨架，但不做红队式批量打靶、自动 PoC 复现或披露流程。
...
3. CTF 与 CVE 都需要有用户可见、可继续迭代的产品入口和状态；CVE 当前只做学习/追踪骨架，
   展示情报、资产命中、学习路径、安全边界和后续 Agent 可接手任务，不做漏洞纵深。
...
1. **P0 · 当前冲刺：完整产品闭环。** 先让 MilkSU 像裸 Pi Agent 一样顺滑完成一个真实产品
   ...
   [产品闭环冲刺](./product-loop-sprint.md)。
   同时补齐 CVE 学习/追踪工作台的最小可见骨架，让它作为一级菜单不再像空模块。
