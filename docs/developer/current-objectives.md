...
| P1 | OTA 版本与更新提示 | Admin 管理可发布版本，App 启动后查询当前频道的新版本并提示更新；发布物、版本清单和真实升级回执尚未实现，按下方独立纵切推进。 |
| P1 | Cloudflare R2 正式发版 | 扩展现有私有签名 workflow，在测试、签名、公证、staple 和 Gatekeeper 通过后上传不可变发布物与清单，再由 Admin 发布版本；正式流水由 CI 持有，审核 Skill 只做检查、触发和回执。 |
...
| 待实现 | Admin OTA 版本管理 | Admin 增加版本列表、草稿/发布/暂停和发布说明；公开只读端点按频道、平台与架构返回当前可用版本。 |
| 待实现 | R2 签名发版流水 | 私有 CI 把正式签名、公证并验证过的发布物、更新清单和校验值上传 Cloudflare R2，回读通过后在 Admin 建立草稿；App 启动查询并完成一次真实旧版到新版升级。 |

### OTA 版本管理与 Cloudflare R2 发版 TODO

目标是把“做出一个签名包”补成用户可用的更新闭环：管理员能发布版本，App 能在启动后发现新版本，
用户确认后完成更新。当前仓库已经有私有 `macOS signed release` workflow，能够完成测试、Developer ID
签名、公证、staple、Gatekeeper 验证并保留 DMG；Admin 当前只有 Worker、D1 和静态管理端，尚无版本表、
R2 绑定或公开更新清单；Desktop 也尚未实现更新查询与安装。
...
1. **Admin 控制面**：在独立私有 `milksu-admin` 中增加版本管理。每个版本至少记录频道、语义版本、
   ...
   已上传对象。
2. **App 更新链路**：主窗口可用后异步读取公开只读版本清单，根据当前频道、平台、架构和语义版本
   判断是否有更新。发现新版本时显示版本号、说明和“更新 / 稍后”；网络失败不阻塞启动，同一版本
   被用户稍后处理后不在同一启动周期重复打扰。用户确认后只接受与清单大小、哈希和签名一致的发布物，
   完成安装与重启后在设置页显示新的版本、commit 和 tracking ID。
...
这条纵切只在以下证据齐全后收口：Admin 可创建并发布真实版本；R2 对象和公开清单已回读且哈希一致；
