...
| BUG-02 | `m3:release-check` 在进入实际检查前要求 `internal/computercap/session-policy.yaml` 被 Git 跟踪，但当前文件不存在且未被跟踪 | 2026-08-04 执行 `npm run m3:release-check`，脚本输出 `Required Computer Use source is not tracked by Git: internal/computercap/session-policy.yaml`；`ls` 与 `git ls-files internal/computercap/session-policy.yaml` 均无结果 | 合并前总检查被第一道门阻断，无法用该脚本证明当前分支可 release-check | 下一批合并前修复：确认该 policy 是应新增的源文件、改脚本依赖，还是旧计划残留；修复后重新跑完整 `m3:release-check` |
