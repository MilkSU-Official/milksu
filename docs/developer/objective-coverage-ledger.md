...
| 分组 | 细项数 | 当前分 | 完成度 |
| --- | ---: | ---: | ---: |
| Coding Agent 与高频替代能力 | 31 | 1,450 / 3,100 | **47%** |
...
不可变 Scope 和窄测试，将 `COD-15` 提到 50%。43% 是当前细项口径的基线。后续只使用
同一张表比较变化。
...
| 主 Agent 可独立准备 | 5 | `COD-24`、`COD-31`、`MEM-08`、`MEM-10`、`DEL-09` |
...
| COD-31 | Computer Use 工具截图接入纯文本模型的辅助视觉回路 | 附件图片已有本地 OCR / 视觉降级；Computer Use 工具结果尚未进入同一路由 | 25% | 文本主模型在 AX 不足时可理解截图并返回可审阅定位依据 |
