...
| 分组 | 细项数 | 当前分 | 完成度 |
| --- | ---: | ---: | ---: |
| Coding Agent 与高频替代能力 | 30 | 1,425 / 3,000 | **48%** |
...
这 35 项最容易被“为什么还不做”混淆，先按主要启动条件分类。分类不改变完成条件。
...
| 主 Agent 可独立准备 | 5 | `COD-24`、`MEM-08`–`MEM-10`、`DEL-09` |
...
| 项目 | 主线 | 产品价值 | 风险 | 启动依赖 | 精确目标 |
| --- | --- | --- | --- | --- | --- |
| `COD-28` | Coding | 中高 | 低至中 | 主 Agent 可准备 | 让一个经过审阅的 Project MCP 在正式打包 Runtime 完成实际工具任务，并保留工具面与审批证据 |
| `CTF-12` | CTF | 高 | 低 | 主 Agent 可准备；真题结果后补 | 建立六赛道固定回归记录格式，把已有 Web 回执登记为 1/6，不虚构其余五项 |
| `MEM-08` | Memory | 高 | 中 | 主 Agent 可实现；真实轨迹后验收 | 推荐理由能够指向具体 Judge、提示、步骤和失败证据，不只显示总结文本 |
| `RUN-08` | Runtime | 高 | 低至中 | 需要用户参与打包 App 重启 | 一个用户可见长任务跨完整 App 重启后，核对会话、任务、PID、端口、日志和恢复结果 |
| `DEL-09` | 交付 | 中 | 低 | 主 Agent 可准备 | 从现有单机测量提出支持矩阵和初始回归阈值，阈值必须可复跑且不冒充多机结果 |

如果五项都从当前分值只提升一档，等权总分理论增加 `125 / 8,900`，约 **1.4 个百分点**。
这只是评估批次容量，不是完成承诺，也不能在真实证据产生前预先加分。
