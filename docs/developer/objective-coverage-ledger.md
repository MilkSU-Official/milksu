...
| BUG-02 | `m3:release-check` 在进入实际检查前要求 `internal/computercap/session-policy.yaml` 被 Git 跟踪，但当前文件不存在且未被跟踪 | 2026-08-04 执行 `npm run m3:release-check`，脚本输出 `Required Computer Use source is not tracked by Git: internal/computercap/session-policy.yaml`；`ls` 与 `git ls-files internal/computercap/session-policy.yaml` 均无结果；随后补入 `internal/computercap/session-policy.yaml` 作为打包 Cua Driver smoke policy，运行时仍由 `internal/computercap/manager.go` 按用户选择的 App 动态生成 per-session policy | 合并前总检查第一道门已解除；仍需重新跑完整 `m3:release-check` 证明没有后续门禁 | 当前批次修复；以 `m3:release-check` 结果决定是否关闭 |
