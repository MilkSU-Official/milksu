...
| COD-25 | 完整 “MilkSU develops MilkSU” Gate | 有多个局部自举任务；产品闭环冲刺期间已补 TopBar 一致性和显式 module 契约、CVE→Coding handoff、Computer Use 快速接入、CTF/CVE 路由保留契约、产品闭环卡生成恢复点入口、用户验收清单、合并状态投影、清单行内操作入口、待补任务 prompt、CVE 当前下一步摘要和多轮前端 test/build/Browser preview 证据；`86ee5d9` 通过完整 `npm run m3:release-check` 并重新生成本机 `MilkSU.app` | 25% | 一次完整 Vue + Go、重启、交付、PR |
