...
GitHub 私有仓库 `MilkSU-Official/milksu`、当前分支 `codex/authorized-learning-foundation`、本地
HEAD、origin HEAD 与 Draft PR #1 head 全部对齐到同一提交，
...
| COD-21 | PR 预览、一次性确认和私有远端限制 | `pull_request_test.go` 覆盖一次性 token、过期、状态变化、私有 MilkSU 远端、窄 `gh pr create` 和读回验证；`CodingChangesPanel.test.ts` 覆盖 UI 先展示仓库/分支/提交/目标，再单独发布，不把内部 confirmation token 显示给用户，后端拒绝过期预览或错误文本回显 token 后清空旧确认并脱敏 token，异常 preview 若指向非 MilkSU 私有仓库则不会进入确认态或调用发布，并在重新准备 PR 时清掉上一轮成功结果，避免旧外部写入状态与新预览混淆；`desktop.test.ts` 覆盖 Wails adapter 会把 workspace、confirmation token、title 和 body 原样传给发布命令，省略标题/正文时只传空字符串而不是 `undefined`；2026-08-05 `MILKSU_GITHUB_PR_STATUS_LIVE_SMOKE=1 npm run test:github-pr-status-live` 只读验证当前 origin 是私有 `MilkSU-Official/milksu`，本地 HEAD、origin HEAD 和 GitHub Draft PR #1 head 全部对齐；同日 `MILKSU_CODING_PR_PUBLISH_LIVE_SMOKE=1 npm run test:coding-pr-publish-live` 用真实打包 App facade 完成 prepare、一次性 token、publish、readback verified 和 token 二次使用拒绝；同日 `MILKSU_CODING_PR_PUBLISH_WEBVIEW_LIVE_SMOKE=1 npm run test:coding-pr-publish-webview-live` 用真实打包 App WebView 打开 Coding 变更面板，点击“准备 PR”和“确认使用现有草稿 PR”，复用并验证私有 Draft PR #1，报告所有 WebView gate 为 true；同日 `MILKSU_CODING_PR_CREATE_LIVE_SMOKE=1 npm run test:coding-pr-create-live` 用真实打包 App facade 在临时新分支上创建 MilkSU 私有 Draft PR、读回验证、拒绝 token 二次使用，并由 launcher 关闭 PR 和删除远端临时分支 | 95% | 可选人工扩样；不再阻塞当前 PR Gate |
| COD-22 | 经确认发布 MilkSU 私有 Draft PR | 当前分支已有真实 GitHub Draft PR #1：`https://github.com/MilkSU-Official/milksu/pull/1`，head repo 为私有 `MilkSU-Official/milksu`，base 为 `main`，PR head 与当前本地/origin HEAD 对齐；`coding-pr-publish-live` 通过真实打包 App facade 复用该 Draft PR 并返回 `verified=true`、`created=false`，同时确认一次性 token 已消费；`coding-pr-publish-webview-live` 进一步通过真实打包 App WebView 的用户可见按钮路径复用并验证 PR #1，报告保存到 `build/test-results/coding-pr-publish-webview-live.json` 与 `build/test-results/coding-pr-publish-webview-live-app-report.json`；`coding-pr-create-live` 已在临时新分支上真实创建 MilkSU 私有 Draft PR，验证 readback 后关闭 PR 并删除远端分支，报告保存到 `build/test-results/coding-pr-create-live.json` 与 `build/test-results/coding-pr-create-live-app-report.json` | 95% | 可选人工扩样；不再阻塞当前 PR Gate |
...
| DONE-COD-06 | 当前分支已有真实 MilkSU 私有 Draft PR 且与本地/origin HEAD 对齐 | `scripts/test-github-pr-status-live.mjs` 新增只读 gated live smoke；2026-08-05 显式 `MILKSU_GITHUB_PR_STATUS_LIVE_SMOKE=1 npm run test:github-pr-status-live` 使用 `gh repo view`、`gh pr list` 和 `git ls-remote` 验证 `MilkSU-Official/milksu` 是 private，当前分支 `codex/authorized-learning-foundation` 的本地 HEAD、origin HEAD 和 GitHub Draft PR #1 head 对齐到同一提交，base 为 `main`，报告保存到 `build/test-results/github-pr-status-live.json` | 不再把“当前分支是否存在真实私有 Draft PR 且是否与远端对齐”作为未知；这仍不是 App 内经用户确认创建/更新 PR，也没有写 GitHub |
| DONE-COD-07 | 打包 App facade 能经一次性确认复用并验证真实 MilkSU 私有 Draft PR | `app_coding_pull_request_smoke.go` 新增只在 `MILKSU_CODING_PR_PUBLISH_SMOKE_RESULT` 存在时运行的 App 内部 smoke；`scripts/test-packaged-coding-pr-publish-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_CODING_PR_PUBLISH_LIVE_SMOKE=1 npm run test:coding-pr-publish-live` 使用真实 `build/bin/MilkSU.app`、当前干净且已 push 的 MilkSU workspace 和 GitHub CLI：App 内部 `PrepareCodingPullRequest` 返回私有 repo、当前 branch/head、existing Draft PR #1 和一次性确认 token；`PublishCodingPullRequest` 使用该 token 复用 PR #1 并返回 `verified=true`、`created=false`；同一 token 二次使用被拒绝；报告不包含 raw confirmation token 或 GitHub token 形态内容 | 不再把“App facade 是否能准备、确认、发布/复用并读回真实 MilkSU Draft PR”作为未完成；这仍不是 WebView 人工点击发布，也没有创建新的 PR |
| DONE-COD-08 | 打包 App WebView 能经用户可见 PR 面板复用并验证真实 MilkSU 私有 Draft PR | `app_coding_pull_request_webview_smoke.go`、`codingPullRequestWebViewSmoke.ts` 和 `scripts/test-packaged-coding-pr-publish-webview-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_CODING_PR_PUBLISH_WEBVIEW_LIVE_SMOKE=1 npm run test:coding-pr-publish-webview-live` 使用真实 `build/bin/MilkSU.app`、当前干净且已 push 的 MilkSU workspace 和 GitHub CLI：WebView 打开 Coding、绑定 `/Users/milksu/code/milksu`、进入右侧变更面板、点击“准备 PR”和“确认使用现有草稿 PR”，复用并验证 `https://github.com/MilkSU-Official/milksu/pull/1`；报告 `build/test-results/coding-pr-publish-webview-live.json` 所有 gate 为 true 且不包含 raw confirmation token 或 GitHub token 形态内容 | 不再把“WebView 中用户可见 PR 发布确认路径”作为未完成；新分支真实创建由 `DONE-COD-09` 关闭，merge 或 ready-for-review 不属于当前门禁 |
