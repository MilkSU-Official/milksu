...
| 分组 | 细项数 | 当前分 | 完成度 |
| --- | ---: | ---: | ---: |
| Coding Agent 与高频替代能力 | 31 | 1,425 / 3,100 | **46%** |
...
Computer Use 工具截图的辅助视觉回路，因此新增 `COD-31` 并校准 `COD-15`。43% 是当前
细项口径的基线，不表示代码变化。后续只使用同一张表比较变化。
...
| 25% | 13 | 只有局部纵切、设计或基础设施 |
| 0% | 23 | 未执行，或当前没有足够证据 |

### 0% / 25% 项的主要启动条件

这 36 项最容易被“为什么还不做”混淆，先按主要启动条件分类。分类不改变完成条件。
...
| 主 Agent 可独立准备 | 6 | `COD-15`、`COD-24`、`COD-31`、`MEM-08`、`MEM-10`、`DEL-09` |
...
| COD-15 | Computer Use 选择当前可见 App / 窗口并生成不可变 Scope | 现有实现硬编码 `com.milksu.app` 与当前 PID；策略和 UI 已有自控基础 | 25% | 用户选择外部 App / 窗口，锁定 bundle、PID、窗口并拒绝切换 |
