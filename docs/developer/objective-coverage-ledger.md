...
> 状态：Current / Post-M3 product-loop merge coverage ledger
>
> 最近已推送代码基线：2026-08-05，`main` `108e0e3`
>
> 最近证据复核：2026-08-05，`108e0e3` 完整 `npm run m3:release-check` 通过并重新生成
> `/Users/milksu/code/milksu/build/bin/MilkSU.app`；后续批次以本文件和提交记录为准
>
> 本文件不是发布说明。它把 `current-objectives.md` 的大项拆成可单独核对的细项，用于保持
> 全局位置。M3 product-loop PR #1 已于 2026-08-05 合并到 `main`；本台账现在用于后续批次
> 选择和已闭环项防重复，避免冲刺 smoke 被误写成全量完成，也避免把已合并 PR 当作当前任务。
...
1. 第一轮只读盘点与共同评估已经完成；M3 product-loop 冲刺已合并。下一批从本台账和
   `current-objectives.md` 中选择有界真实闭环，不从旧冲刺文案重复打开已闭环项。
...
`tool_result` hook 并加缓存测试，将 `COD-31` 提到 50%；2026-08-05 packaged Coding
bridge live smoke 用真实打包 sidecar、真实 Calculator 截图和确定性本地 Provider 证明
纯文本主模型能消费辅助视觉证据后发起点击，将 `COD-31` 提到 75%；GUI 任务在 Computer Use 未启用时
...
同日补 Settings 页面 Computer Use 权限/签名诊断和只读重新检测入口，并用
`MILKSU_SETTINGS_COMPUTER_USE_WEBVIEW_LIVE_SMOKE=1 npm run test:settings-computer-use-webview-live`
证明真实打包 App WebView 可打开设置页、展示 Computer Use 区、点击重新检测并读回当前
`accessibility=true / screenRecording=true`，将 `COD-16` 从 90% 调到 100%。
...
打包 App 导入 Codex fixture 后可搜索并保持脱敏，将 `COD-32` 调到 95%。2026-08-05
...
`RUN-08` 从 25% 调到 75%。同日
`MILKSU_CODING_BACKGROUND_WEBVIEW_RECOVERY_LIVE_SMOKE=1 npm run test:coding-background-webview-recovery-live`
用真实打包 WebView 完成 Coding → 终端 → 后台任务 UI 内输入命令、点击运行、App 退出后
heartbeat 继续增长、重启后 UI 显示恢复提示/同一 PID/log tail，再点击停止，报告
`build/test-results/coding-background-webview-recovery-live.json`，将 `COD-18` 从 75% 调到 95%、
`RUN-08` 从 75% 调到 100%。同日 `MILKSU_VULN_PRACTICE_LIVE_SMOKE=1 npm run test:vuln-practice-live`
用真实打包 App 对本地 benign Docker Compose 练习目录完成 start/status/stop/cleanup，报告
`build/test-results/vuln-practice-live.json`，且 `docker ps` 按测试 label 无残留容器；这关闭
`DONE-CVE-06` 的 packaged Docker 生命周期缺口，但不等价于真实 Vulhub 漏洞练习、PoC 复现
或资产验证。同日 `MILKSU_VULN_LEARNING_WRITEBACK_LIVE_SMOKE=1 npm run test:vuln-learning-writeback-live`
使用真实打包 App 证明 `CVE-2023-46604` 的 `cve-tracking` 研究档案可创建/复用，用户确认的
Coding 结论可写入 Go `learning.recorded` 事件并从 projection 恢复读取；这把 `COD-25` 从
45% 调到 50%，但仍不是完整 MilkSU 源码自举、真实外部 Provider 质量、私有远端 PR Gate 或
资产验证。随后 `MILKSU_VULN_LEARNING_WRITEBACK_WEBVIEW_LIVE_SMOKE=1 npm run test:vuln-learning-writeback-webview-live`
使用真实打包 App WebView 打开 CVE 工作台、选中 `CVE-2023-46604`、通过 UI 导入 Coding
结论并读回正式研究档案 `job_56077706-4a0d-4af7-aa2b-67ce585c02c6` 和 1 条学习记录；这关闭
“导入 Coding 结论没有打包 App UI 链路证据”的缺口。随后 `MILKSU_VULN_ASSET_VERIFICATION_WEBVIEW_LIVE_SMOKE=1 npm run test:vuln-asset-verification-webview-live`
使用真实打包 App WebView 打开 CVE 工作台、选中 `CVE-2023-46604`、通过 UI 新增用户确认
资产，并读回正式研究档案 `job_6f83d5ce-5791-4b78-87a2-c529b33b51ae` 和 1 条
`asset_verification.recorded` 投影；这关闭“资产验证只能留在本地列表/没有打包 App UI 链路证据”
的缺口。随后 `MILKSU_VULN_PRACTICE_DIRECTORY_WEBVIEW_LIVE_SMOKE=1 npm run test:vuln-practice-directory-webview-live`
使用真实打包 App WebView 打开 CVE 工作台、选中 `CVE-2023-46604`、确认练习计划，并绑定
来自 `github.com/vulhub/vulhub` commit `aeaf65793f147f29bd50841ef77f4e9cad07ecc7` 的
`activemq/CVE-2023-46604` 用户目录；这关闭真实 Vulhub 用户目录 UI 绑定缺口，但仍不等于
真实漏洞复现、外部资产可被利用、对真实 Vulhub 目录启动 Docker，或任何 exploit/漏洞触发输入。
随后 `MILKSU_MILKSU_SOURCE_SELF_BOOTSTRAP_LIVE_SMOKE=1 npm run test:milksu-source-self-bootstrap-live`
使用真实 MilkSU 源码隔离 clone，让 Coding runtime 新增真实 Node 脚本/测试、Go 源码/测试和
前端 TS/Vitest 文件，运行 `node --test`、`go test ./internal/sourcebootstrap` 与
`npm --prefix app run test -- --run src/lib/sourceSelfBootstrapSmoke.test.ts`，再由真实打包 App
Git facade commit/push 到隔离 bare remote，将 `COD-25` 从 55% 调到 60%；这仍不是真实外部
Provider、当前主工作区自改、私有远端 PR Gate 或 Hosted PR 发布。
随后 `MILKSU_GITHUB_PR_STATUS_LIVE_SMOKE=1 npm run test:github-pr-status-live` 只读验证
GitHub 私有仓库 `MilkSU-Official/milksu`、合并前分支 `codex/authorized-learning-foundation`、本地
HEAD、origin HEAD 与 Draft PR #1 head 当时全部对齐到同一提交，
将 `COD-21` 从 50% 调到 60%，`COD-22` 从 0% 调到 25%；这仍不等于 App 内经用户确认创建或
更新 PR。
随后 `MILKSU_CODING_PR_PUBLISH_LIVE_SMOKE=1 npm run test:coding-pr-publish-live` 使用真实打包
`MilkSU.app` 调用 `PrepareCodingPullRequest` 与 `PublishCodingPullRequest`，在已有私有
Draft PR #1 的前提下完成 preview、一次性 confirmation token、publish 复用、readback verified
和 token 二次使用拒绝，将 `COD-21` 从 60% 调到 75%，`COD-22` 从 25% 调到 50%；当时仍不是
WebView 人工点击发布，也没有创建新的 PR。
随后 `MILKSU_CODING_PR_PUBLISH_WEBVIEW_LIVE_SMOKE=1 npm run test:coding-pr-publish-webview-live`
使用真实打包 `MilkSU.app` WebView 进入 Coding 变更面板，点击“准备 PR”与“确认使用现有草稿
PR”，复用并验证同一私有 Draft PR #1，将 `COD-21` 从 75% 调到 85%，`COD-22` 从 50% 调到
75%；这仍没有在新分支上创建新的 PR。
随后 `MILKSU_CODING_PR_CREATE_LIVE_SMOKE=1 npm run test:coding-pr-create-live` 使用真实打包
`MilkSU.app` 在临时新分支上经 App facade 创建 MilkSU 私有 Draft PR，读回 verified/open/draft
状态，确认一次性 token 二次使用被拒绝，并由 launcher 关闭 PR 与删除远端临时分支，将
`COD-21` 从 85% 调到 95%，`COD-22` 从 75% 调到 95%，`COD-25` 从 65% 调到 70%；后续不再把
“新分支真实创建 PR”作为当前 PR Gate 缺口。
后续只使用同一张表比较变化。
...
| 60% | 1 | 已有真实源码隔离 clone 联动证据，但仍缺最终 Gate |
...
| COD-16 | Computer Use 一次性系统权限真实验收 | UI 已说明 App 管理不能替代辅助功能/屏幕录制，提供请求系统权限和重新检测入口；`1698e39` 增加从产品闭环卡直达权限/可见窗口接入面板的用户路径；2026-08-04 又把缺系统权限、待选择窗口、可启动和已接入分开展示，避免用户把“App 管理”或“已检测到窗口”误解成 Computer Use 已完成接入；2026-08-05 在用户已授权系统权限并放行本机拦截后，packaged proxy live smoke 成功读取外部 Calculator 窗口截图并执行点击；随后 `MILKSU_COMPUTER_USE_APP_LIVE_SMOKE=1 npm run test:computer-use-app-live` 用真实 `MilkSU.app` App facade 完成 list/start/status/descriptor/stop，报告保存到 `build/test-results/computer-use-app-live.json`，证明权限可支撑 App 层启动外部窗口会话；同日真实 MilkSU UI 点击验收显示辅助功能/屏幕录制按钮均 disabled 且状态可启动，说明当前授权支撑 UI 内外部窗口会话启动；随后同一隔离 App data 重启 MilkSU，按钮仍可读、会话回到 `可启动`，没有把旧 Calculator PID/window 当作仍授权会话；同日 Settings 页面新增 Computer Use 权限/签名诊断和只读重新检测入口，`settings-computer-use-webview-live` 用真实打包 App WebView 证明设置页可打开、权限区可见、点击重新检测后读回 `accessibility=true / screenRecording=true`，且报告不含 key 形态内容 | 100% | — |
...
| COD-21 | PR 预览、一次性确认和私有远端限制 | `pull_request_test.go` 覆盖一次性 token、过期、状态变化、私有 MilkSU 远端、窄 `gh pr create` 和读回验证；`CodingChangesPanel.test.ts` 覆盖 UI 先展示仓库/分支/提交/目标，再单独发布，不把内部 confirmation token 显示给用户，后端拒绝过期预览或错误文本回显 token 后清空旧确认并脱敏 token，异常 preview 若指向非 MilkSU 私有仓库则不会进入确认态或调用发布，并在重新准备 PR 时清掉上一轮成功结果，避免旧外部写入状态与新预览混淆；`desktop.test.ts` 覆盖 Wails adapter 会把 workspace、confirmation token、title 和 body 原样传给发布命令，省略标题/正文时只传空字符串而不是 `undefined`；2026-08-05 `MILKSU_GITHUB_PR_STATUS_LIVE_SMOKE=1 npm run test:github-pr-status-live` 只读验证合并前 origin 是私有 `MilkSU-Official/milksu`，当时本地 HEAD、origin HEAD 和 GitHub Draft PR #1 head 全部对齐；同日 `MILKSU_CODING_PR_PUBLISH_LIVE_SMOKE=1 npm run test:coding-pr-publish-live` 用真实打包 App facade 完成 prepare、一次性 token、publish、readback verified 和 token 二次使用拒绝；同日 `MILKSU_CODING_PR_PUBLISH_WEBVIEW_LIVE_SMOKE=1 npm run test:coding-pr-publish-webview-live` 用真实打包 App WebView 打开 Coding 变更面板，点击“准备 PR”和“确认使用现有草稿 PR”，复用并验证私有 Draft PR #1，报告所有 WebView gate 为 true；同日 `MILKSU_CODING_PR_CREATE_LIVE_SMOKE=1 npm run test:coding-pr-create-live` 用真实打包 App facade 在临时新分支上创建 MilkSU 私有 Draft PR、读回验证、拒绝 token 二次使用，并由 launcher 关闭 PR 和删除远端临时分支 | 95% | 可选人工扩样；PR #1 已合并，不再阻塞当前 PR Gate |
| COD-22 | 经确认发布 MilkSU 私有 Draft PR | 合并前分支已有真实 GitHub Draft PR #1：`https://github.com/MilkSU-Official/milksu/pull/1`，head repo 为私有 `MilkSU-Official/milksu`，base 为 `main`，PR head 与当时本地/origin HEAD 对齐；`coding-pr-publish-live` 通过真实打包 App facade 复用该 Draft PR 并返回 `verified=true`、`created=false`，同时确认一次性 token 已消费；`coding-pr-publish-webview-live` 进一步通过真实打包 App WebView 的用户可见按钮路径复用并验证 PR #1，报告保存到 `build/test-results/coding-pr-publish-webview-live.json` 与 `build/test-results/coding-pr-publish-webview-live-app-report.json`；`coding-pr-create-live` 已在临时新分支上真实创建 MilkSU 私有 Draft PR，验证 readback 后关闭 PR 并删除远端分支，报告保存到 `build/test-results/coding-pr-create-live.json` 与 `build/test-results/coding-pr-create-live-app-report.json` | 95% | 可选人工扩样；PR #1 已合并，新发布任务另起 PR |
...
| COD-25 | 完整 “MilkSU develops MilkSU” Gate | 有多个局部自举任务；产品闭环冲刺期间已补 TopBar 一致性和显式 module 契约、CVE→Coding handoff、Computer Use 快速接入、CTF/CVE 路由保留契约、产品闭环卡生成恢复点入口、用户验收清单、合并状态投影、清单行内操作入口、待补任务 prompt、CVE 当前下一步摘要、原生 App 验收接力卡和多轮前端 test/build/Browser preview 证据；`86ee5d9` 通过完整 `npm run m3:release-check` 并重新生成本机 `MilkSU.app`；2026-08-04 在 compact 控件视觉收敛后再次运行最新 HEAD 的 `npm run m3:release-check`，先发现 Go 样式契约仍期待旧行高，修正后重跑通过并重新生成 `/Users/milksu/code/milksu/build/bin/MilkSU.app`，Coding delivery fixture 仍为 score 100、外部 Provider cost 0；随后 Coding 产品闭环卡新增“推荐小自举任务”复制入口，把低风险、用户可见、测试/build/Browser 验证、登记非阻塞问题、commit/push 和 Provider Key 边界写进下一轮 Agent prompt，并由 `CodingProductLoopPanel.test.ts` 与 Browser preview 证明入口可见；CVE 情报源刷新也改成可验证的本机快照复核与只读 Feed 导入接力，避免把静态样例误呈现成实时情报能力；最新在 Coding 合并状态卡下新增 `Coding 待补证明` 短列表，直接列出当前未完成验收项并提供面板/恢复入口，避免下一轮 Agent 或用户从长清单里猜缺口；2026-08-05 在 `108e0e3` 再次执行完整 `npm run m3:release-check`，Go/vet/Node/Vitest/lint/build/sidecar smoke/Coding delivery/docs build/Wails build/codesign 均通过，并重新生成 `/Users/milksu/code/milksu/build/bin/MilkSU.app`；这覆盖了 M3 product-loop squash merge 后的 main 基线；同日 `MILKSU_CODING_SELF_BOOTSTRAP_LIVE_SMOKE=1 npm run test:coding-self-bootstrap-live` 使用真实打包 App 导入/搜索外部历史、把用户确认相关历史送进 Coding delivery prompt，Agent 在隔离 Git fixture 中完成 4 个文件修改、测试、恢复和 compaction，最后真实打包 App Git facade 把改动 stage/commit/push 到本地 bare remote，报告 `build/test-results/coding-self-bootstrap-live.json`；同日 `MILKSU_MILKSU_SOURCE_SELF_BOOTSTRAP_LIVE_SMOKE=1 npm run test:milksu-source-self-bootstrap-live` 使用真实 MilkSU 源码隔离 clone，Coding runtime 在 clone 中新增 Node 脚本/测试、Go 源码/测试和前端 TS/Vitest 文件，运行 `node --test tests/scripts/milksu-source-self-bootstrap-smoke.test.mjs`、`go test ./internal/sourcebootstrap` 与 `npm --prefix app run test -- --run src/lib/sourceSelfBootstrapSmoke.test.ts`，再由真实打包 App Git facade commit/push 到隔离 bare remote，报告 `build/test-results/milksu-source-self-bootstrap-live.json`；同日 CVE learning writeback 纵切作为真实 MilkSU 源码 Vue + Go 修改落地，`VulnPage.vue` 的“导入 Coding 结论”不再只写 localStorage，而是经 Wails/Go 创建 `cve-tracking` 档案并记录 `learning.recorded`，最终打包 App facade smoke 报告 `build/test-results/vuln-learning-writeback-live.json`；随后 packaged WebView UI smoke 报告 `build/test-results/vuln-learning-writeback-webview-live.json` 证明打包 App 前端能自动进入 CVE、选中 `CVE-2023-46604`、通过 UI 导入 Coding 结论并读回正式研究档案；同日 `coding-pr-publish-webview-live` 报告证明打包 App WebView 能从 Coding 变更面板经用户可见按钮复用并验证 MilkSU 私有 Draft PR #1；同日 `coding-pr-create-live` 报告证明真实打包 App facade 能在新分支创建 MilkSU 私有 Draft PR、读回验证并自动关闭清理；同日 `coding-background-webview-recovery-live` 报告证明真实打包 App WebView 能从 Coding 终端面板启动后台长任务、重启后查看恢复状态并停止 | 72% | 仍缺真实外部 Provider 质量、Browser/Computer Use 预览复核、当前主工作区自改交付 |
...
| OBS-11 | Runtime 已能投影后台任务 PID、端口、日志并在 Sidecar 重启后恢复；2026-08-05 又通过真实打包 App 后台恢复 smoke 和 WebView UI 长任务恢复 smoke | `RUN-08` 已到 100%，不再重复实现 Sidecar 恢复、packaged deterministic smoke 或用户可见 UI 长任务恢复；后续只做真实项目扩样或 `COD-19` 交互式 PTY 负向 |
...
| OBS-13 | macOS “App 管理”权限不等于 Computer Use 所需 Accessibility；截图可用也不代表 AX 可用；`CodingComputerUsePanel.test.ts` 已覆盖缺辅助功能/屏幕录制时启动禁用、可请求系统权限，并在 Computer Use 不可用时禁用请求按钮；2026-08-04 面板状态已区分缺系统权限、待选择窗口、可启动和已接入当前任务；2026-08-05 `DONE-CU-06` 已补 Settings 页面只读重新检测与真实打包 App WebView 验收 | 不再把 COD-16 的权限重新检测当作未完成；后续只在出现新的系统权限复现或签名发行问题时重开 |
...
| DONE-M3-01 | 当前代码基线完整工程门禁通过 | 2026-08-05 在 `108e0e3` 执行 `npm run m3:release-check`，覆盖 Go test/vet、Node tests、前端 Vitest/lint/build、Sidecar smoke、Coding delivery fixture、docs build、Wails build、ad-hoc codesign 验证和 `check-macos-signing.mjs`，最终输出 `M3 engineering release checks passed.` 并生成 `/Users/milksu/code/milksu/build/bin/MilkSU.app` | 不再重复质疑“当前 HEAD 能否编译/打包”；但这不等价于 COD-25 的完整自举任务、Developer ID 签名、公证、升级或外部 Beta 门禁 |
| DONE-M3-02 | M3 product-loop PR #1 已合并到 main | 2026-08-05，PR #1 已从 `codex/authorized-learning-foundation` squash merge 到 `main`，合并提交为 `108e0e3 feat: add authorized-learning foundation and M3-A evidence slice`；合并后 `main` 与 `origin/main` 对齐 | 不再把 `codex/authorized-learning-foundation` 或 PR #1 当作打开中的当前交付目标；后续 PR/分支按新批次另起 |
...
| DONE-RUN-01 | 打包 App 能恢复跨 App 重启仍在运行的 Coding 后台任务并停止它 | `app_coding_background_recovery_smoke.go` 新增只在 `MILKSU_CODING_BACKGROUND_RECOVERY_SMOKE_RESULT` 存在时运行的 App 内部 smoke；`scripts/test-packaged-coding-background-recovery-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_CODING_BACKGROUND_RECOVERY_LIVE_SMOKE=1 npm run test:coding-background-recovery-live` 使用真实 `build/bin/MilkSU.app` 和隔离 App data：首次启动 `sh bg-worker.sh` 后台任务并观测 log tail/heartbeat，App 退出后 heartbeat 继续增长；第二次启动恢复同一 task/PID，再由 App facade 停止任务；报告保存到 `build/test-results/coding-background-recovery-live.json`，start/recover 子报告保存到 `build/test-results/coding-background-recovery-live-start.json` 和 `build/test-results/coding-background-recovery-live-recover.json` | 不再把“打包 App deterministic 后台任务能否跨 App 重启恢复并停止”作为未完成；用户可见 UI 长任务恢复由 `DONE-RUN-02` 关闭，交互式 PTY 负向仍归 `COD-19` |
| DONE-RUN-02 | 打包 WebView UI 能启动后台长任务、重启后恢复显示并停止 | `app_coding_background_recovery_webview_smoke.go` 新增 gated WebView smoke hook；`codingBackgroundRecoveryWebViewSmoke.ts` 通过真实打包 App 页面打开 Coding 工作区、切到终端后台任务、输入 `sh bg-ui-worker.sh` 并点击运行，第二次启动同一 App data 后再从 UI 观察恢复提示、同一 PID 和 log tail marker，并点击停止；`scripts/test-packaged-coding-background-webview-recovery-live.mjs` 会在失败时清理 worker PID。2026-08-05 显式 `MILKSU_CODING_BACKGROUND_WEBVIEW_RECOVERY_LIVE_SMOKE=1 npm run test:coding-background-webview-recovery-live` 通过，报告 `build/test-results/coding-background-webview-recovery-live.json` 显示 `visibleTasksPanelStartedLongTask`、`taskSurvivedAppQuit`、`visibleTasksPanelRecoveredAfterRestart`、`visibleRecoveryBannerObserved`、`recoveredSamePID`、`visibleStopButtonStoppedTask` 和 `noProviderCredentialLeak` 均为 true | 不再把“用户可见 UI 手动启动真实长任务、重启后查看恢复状态并停止”作为 `COD-18` / `RUN-08` 的未完成项；交互式 PTY 不可重连仍按 `COD-19` 负向验收 |
...
| DONE-COD-04 | 相关历史参与一次自举式 Coding delivery，并由打包 App Git facade 交付到隔离远端 | `app_coding_git_delivery_smoke.go` 新增只在 `MILKSU_CODING_GIT_DELIVERY_SMOKE_RESULT` 存在时运行的 packaged App Git smoke；`scripts/test-coding-agent-delivery.mjs` 支持用户确认相关历史上下文和真实 Git fixture；`scripts/test-packaged-coding-self-bootstrap-live.mjs` 串联真实打包 App 外部历史导入/搜索 → Coding delivery fixture → 真实打包 App stage/commit/push；2026-08-05 显式 `MILKSU_CODING_SELF_BOOTSTRAP_LIVE_SMOKE=1 npm run test:coding-self-bootstrap-live` 通过，报告 `build/test-results/coding-self-bootstrap-live.json` 显示 4 个文件修改、测试通过、相关历史 token 进入 24 次 provider request、commit `af45b3bc00cb` 推送到隔离 bare remote 且 workspace clean | 不再把“相关历史能否参与一次实际 Coding 交付”或“打包 App Git facade 能否对隔离仓库 stage/commit/push”作为未完成；这仍不是 MilkSU 源码 Vue+Go、自主 PR 或真实外部 Provider 质量 |
| DONE-COD-05 | Coding runtime 能在真实 MilkSU 源码隔离 clone 中完成 Node + Go + 前端 edit/test，并由打包 App Git facade 交付 | `scripts/test-packaged-milksu-source-self-bootstrap-live.mjs` 的 gated live smoke 从当前 MilkSU 仓库克隆真实源码到隔离目录，Coding runtime 使用打包 sidecar node 读取 `package.json` 和 `app/src/App.vue`，写入 `scripts/lib/milksu-source-self-bootstrap-smoke.mjs`、`tests/scripts/milksu-source-self-bootstrap-smoke.test.mjs`、`internal/sourcebootstrap/smoke.go`、`internal/sourcebootstrap/smoke_test.go`、`app/src/lib/sourceSelfBootstrapSmoke.ts` 与 `app/src/lib/sourceSelfBootstrapSmoke.test.ts`，运行 `node --test tests/scripts/milksu-source-self-bootstrap-smoke.test.mjs`、`go test ./internal/sourcebootstrap` 与 `npm --prefix app run test -- --run src/lib/sourceSelfBootstrapSmoke.test.ts`，再用真实 `build/bin/MilkSU.app` 的 Git facade stage/commit/push 到隔离 bare remote；2026-08-05 显式 `MILKSU_MILKSU_SOURCE_SELF_BOOTSTRAP_LIVE_SMOKE=1 npm run test:milksu-source-self-bootstrap-live` 通过，报告 `build/test-results/milksu-source-self-bootstrap-live.json` 显示 `codingRuntimeEditedGoSource=true`、`codingRuntimeEditedFrontendSource=true`、`codingRuntimeRanGoTest=true`、`codingRuntimeRanFrontendTest=true`、`providerPlanConsumed=true`、`toolFailures=0`、`remoteHead=0688153153f4`、工作区 clean，且实际比较运行前后主工作区 `git status` 确认 active worktree 未被 smoke 污染 | 不再把 COD-25 说成只有 toy fixture 或缺确定性 Vue+Go 源码 clone 联动；这仍不是当前主工作区自改、真实外部 Provider、私有远端 PR 或 Hosted PR 发布 |
| DONE-COD-06 | 合并前分支已有真实 MilkSU 私有 Draft PR 且与本地/origin HEAD 对齐 | `scripts/test-github-pr-status-live.mjs` 新增只读 gated live smoke；2026-08-05 显式 `MILKSU_GITHUB_PR_STATUS_LIVE_SMOKE=1 npm run test:github-pr-status-live` 使用 `gh repo view`、`gh pr list` 和 `git ls-remote` 验证 `MilkSU-Official/milksu` 是 private，`codex/authorized-learning-foundation` 的本地 HEAD、origin HEAD 和 GitHub Draft PR #1 head 当时对齐到同一提交，base 为 `main`，报告保存到 `build/test-results/github-pr-status-live.json` | 这是 PR #1 合并前的只读验收事实；PR #1 已合并，后续不要把该分支/PR 当作当前打开目标 |
| DONE-COD-07 | 打包 App facade 能经一次性确认复用并验证真实 MilkSU 私有 Draft PR | `app_coding_pull_request_smoke.go` 新增只在 `MILKSU_CODING_PR_PUBLISH_SMOKE_RESULT` 存在时运行的 App 内部 smoke；`scripts/test-packaged-coding-pr-publish-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_CODING_PR_PUBLISH_LIVE_SMOKE=1 npm run test:coding-pr-publish-live` 使用真实 `build/bin/MilkSU.app`、当时干净且已 push 的 MilkSU workspace 和 GitHub CLI：App 内部 `PrepareCodingPullRequest` 返回私有 repo、current branch/head、existing Draft PR #1 和一次性确认 token；`PublishCodingPullRequest` 使用该 token 复用 PR #1 并返回 `verified=true`、`created=false`；同一 token 二次使用被拒绝；报告不包含 raw confirmation token 或 GitHub token 形态内容 | 不再把“App facade 是否能准备、确认、发布/复用并读回真实 MilkSU Draft PR”作为未完成；PR #1 已合并，新 PR 需由后续批次另起 |
| DONE-COD-08 | 打包 App WebView 能经用户可见 PR 面板复用并验证真实 MilkSU 私有 Draft PR | `app_coding_pull_request_webview_smoke.go`、`codingPullRequestWebViewSmoke.ts` 和 `scripts/test-packaged-coding-pr-publish-webview-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_CODING_PR_PUBLISH_WEBVIEW_LIVE_SMOKE=1 npm run test:coding-pr-publish-webview-live` 使用真实 `build/bin/MilkSU.app`、当时干净且已 push 的 MilkSU workspace 和 GitHub CLI：WebView 打开 Coding、绑定 `/Users/milksu/code/milksu`、进入右侧变更面板、点击“准备 PR”和“确认使用现有草稿 PR”，复用并验证 `https://github.com/MilkSU-Official/milksu/pull/1`；报告 `build/test-results/coding-pr-publish-webview-live.json` 所有 gate 为 true 且不包含 raw confirmation token 或 GitHub token 形态内容 | 不再把“WebView 中用户可见 PR 发布确认路径”作为未完成；PR #1 已合并，新分支真实创建由 `DONE-COD-09` 关闭 |
| DONE-COD-09 | 打包 App facade 能在新分支真实创建并清理 MilkSU 私有 Draft PR | `app_coding_pull_request_create_smoke.go` 和 `scripts/test-packaged-coding-pr-create-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_CODING_PR_CREATE_LIVE_SMOKE=1 npm run test:coding-pr-create-live` 使用真实 `build/bin/MilkSU.app`、GitHub CLI 和 MilkSU 私有仓库临时分支：launcher 克隆当前仓库、创建并推送临时 `codex/pr-create-smoke-*` 分支，App 内部 `PrepareCodingPullRequest` 返回无 existing PR 的私有 repo 预览与一次性 token，`PublishCodingPullRequest` 创建 Draft PR、读回 `verified=true/open/draft`，同一 token 二次使用被拒绝；launcher 随后关闭临时 PR 并删除远端临时分支；报告 `build/test-results/coding-pr-create-live.json` 与 `build/test-results/coding-pr-create-live-app-report.json` 所有 gate 为 true，且不包含 raw confirmation token 或 GitHub token 形态内容 | 不再把“新分支真实创建 PR”作为当前 PR Gate 缺口；这不 merge、不 ready-for-review，也不向引用开源项目创建 PR |
| DONE-CVE-01 | App facade 与打包 App 的真实 NVD 单 CVE 下载、事实提取和原始快照持久化 | `app_vuln_live_test.go` 新增 `TestLiveAppFetchNVDCVEPersistsSnapshot`；默认跳过，显式 `MILKSU_LIVE_CVE_APP_SMOKE=1 go test . -run TestLiveAppFetchNVDCVEPersistsSnapshot -count=1 -v` 已真实访问 NVD `CVE-2024-3400`，验证 `retrievedAt`、App data 下 `vuln/feed-snapshots/nvd/*.json`、返回 body 与磁盘 body 一致、SHA-256 一致、文件权限 `0600`；2026-08-05 新增 `scripts/test-packaged-vuln-feed-live.mjs`，显式 `MILKSU_VULN_FEED_LIVE_SMOKE=1 npm run test:vuln-feed-live` 使用真实 `build/bin/MilkSU.app` 隔离进程拉取 NVD `CVE-2024-3400`，报告保存到 `build/test-results/vuln-feed-live.json`，snapshot 证据复制到 `build/test-results/vuln-feed-live-snapshot.json`，结果包含 `retrievedAt=2026-08-04T18:31:12Z`、`severity=CRITICAL`、`baseScore=10`、snapshot `sha256=ef863c16857bec109adf9b6006910468a94ea2791f4a4f51a218073712531b79`，且报告不包含原始 feed body | 不再把 “CVE 真实 NVD 下载、选中 CVE facts、来源时间和 App data snapshot” 视为未实现，也不要说 CVE 只有四条 mock；资产验证 UI 写回由 `DONE-CVE-09` 关闭，真实 Vulhub 用户目录 UI 绑定由 `DONE-CVE-10` 关闭 |
...
| DONE-CVE-03 | 打包 App 多源 CVE 情报与 Vulhub 练习目录匹配 | `app_vuln_feed_matrix_smoke.go` 和 `scripts/test-packaged-vuln-feed-matrix-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_VULN_FEED_MATRIX_LIVE_SMOKE=1 npm run test:vuln-feed-matrix-live` 使用真实 `build/bin/MilkSU.app` 隔离进程为 `CVE-2023-46604` 同步 NVD、FIRST EPSS、CISA KEV 和 Vulhub practice catalog，报告保存到 `build/test-results/vuln-feed-matrix-live.json`，四份 snapshot 证据复制到 `build/test-results/vuln-feed-matrix-live-nvd.json`、`build/test-results/vuln-feed-matrix-live-first-epss.json`、`build/test-results/vuln-feed-matrix-live-cisa-kev.json` 和 `build/test-results/vuln-feed-matrix-live-vulhub-practice-catalog.json`；结果包含 EPSS `0.996540000`、KEV 命中、Vulhub `activemq/CVE-2023-46604/docker-compose.yml`，且报告不包含原始 feed body 或 key 形态内容 | 不再把“CVE 是否能从多个真实源同步并匹配练习目录”作为未实现；Docker 生命周期由 `DONE-CVE-06` 关闭，研究写回由 `DONE-CVE-07/08` 关闭，资产验证 UI 写回由 `DONE-CVE-09` 关闭，真实 Vulhub 用户目录 UI 绑定由 `DONE-CVE-10` 关闭 |
| DONE-CVE-04 | CVE 设置页主入口能执行当前 CVE 四源同步矩阵并逐源留状态 | `VulnerabilityIntelSettingsPanel.vue` 的“同步当前 CVE”现在并发调用 NVD、FIRST EPSS、CISA KEV 和 Vulhub；同步后恢复用户原先选中的 CVE，逐源记录成功/失败，Vulhub 结果显示当前 CVE 是否匹配练习目录；`VulnPage.test.ts` 覆盖 NVD timeout 时 EPSS、KEV 和 Vulhub 仍成功留证据并显示 `3/4 个来源成功`、`当前 CVE 已匹配 pan-os/CVE-2024-3400`，且不出现 `Judge verified`；2026-08-05 Browser 在 `http://127.0.0.1:1420/` 验证 CVE 首页默认不显示 Feed 缓存台，打开设置后可见四源矩阵主按钮、单源重试按钮和缓存状态，Console 无 error/warn | 不再把“UI 内多源同步状态”作为未实现；真实打包 App UI/Wails 公网同步由 `DONE-CVE-05` 关闭，Docker 生命周期由 `DONE-CVE-06` 关闭，研究写回由 `DONE-CVE-07/08` 关闭，资产验证 UI 写回由 `DONE-CVE-09` 关闭，真实 Vulhub 用户目录 UI 绑定由 `DONE-CVE-10` 关闭 |
| DONE-CVE-05 | 真实打包 MilkSU UI 点击 CVE 四源同步并落盘 snapshot | 2026-08-05 用真实 `/Users/milksu/code/milksu/build/bin/MilkSU.app` 和隔离 `MILKSU_APPDATA_DIR=/tmp/milksu-cve-ui-live.CjnNtr/app-data` 启动 App；通过 Computer Use 进入 CVE → 设置，点击 `同步当前 CVE 的 NVD、FIRST EPSS、CISA KEV 和 Vulhub`；UI 恢复后显示 `当前 CVE 情报矩阵同步完成：4/4 个来源成功`，当前 CVE `CVE-2024-3400` 的来源证据新增 NVD、FIRST EPSS、CISA KEV 原始快照，Vulhub catalog 显示 249 个候选且当前 CVE 未匹配；文件元数据确认隔离 app-data 下存在四份 snapshot：`vulhub-practice-catalog` 351156 bytes、`cisa-kev` 1574298 bytes、`first-epss` 202 bytes、`nvd` 14101 bytes；报告保存到 `build/test-results/vuln-ui-feed-matrix-live.json`，截图保存到 `build/test-results/vuln-ui-feed-matrix-live-after-sync.png` | 不再把“CVE 设置页真实点击四源同步是否会通过 Wails 访问公网并落盘来源时间/snapshot”作为未完成；真实 Vulhub 用户目录 UI 绑定由 `DONE-CVE-10` 关闭，如需时可做人工/Computer Use 点击复核 |
| DONE-CVE-06 | 打包 App 能启动、观察、停止并清理本地 CVE Docker Compose 练习环境 | `internal/vuln/practice.go` 新增受控本地 Compose 生命周期：用户选定本地目录、校验 compose 文件、拒绝 host network/privileged/docker.sock、Docker 命令环境不继承 Provider Key；`app_vuln_practice_smoke.go` 和 `scripts/test-packaged-vuln-practice-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_VULN_PRACTICE_LIVE_SMOKE=1 npm run test:vuln-practice-live` 使用真实 `build/bin/MilkSU.app`、隔离 App data 和 benign local Compose fixture，完成 start/status/stop/cleanup，报告保存到 `build/test-results/vuln-practice-live.json` 与 `build/test-results/vuln-practice-live-app-report.json`，报告权限 `0600`，`docker ps --filter label=org.milksu.fixture=vuln-practice-live` 无残留容器 | 不再把“打包 App 是否能对本地 CVE Docker Compose 练习环境执行生命周期”作为未完成；真实 Vulhub 用户目录 UI 绑定由 `DONE-CVE-10` 关闭。本行不运行 exploit、漏洞触发输入、外部目标流量，也不证明资产存在漏洞 |
| DONE-CVE-07 | 用户确认的 Coding 结论可写入正式 CVE 研究档案 | `TrackingWorkspaceRequest`、`EnsureCVETrackingWorkspace` 和 `EnsureVulnTrackingWorkspace` 新增 `cve-tracking` 研究档案；`VulnPage.vue` 的“导入 Coding 结论”现在先保留本地笔记，再经 Wails/Go 创建或复用对应 CVE 的 `vuln.research` job，并调用 `RecordVulnLearning` 写入 `learning.recorded`；UI 显示正式研究档案 ID、同步时间和学习记录数；`TestCVETrackingWorkspaceRecordsUserConfirmedLearning` 覆盖 origin scope 规范化、学习记录投影和重复请求复用同一 job；`VulnPage.test.ts` 与 `desktop.test.ts` 覆盖前端调用；2026-08-05 在完整 `npm run m3:release-check` 后再次执行 `MILKSU_VULN_LEARNING_WRITEBACK_LIVE_SMOKE=1 npm run test:vuln-learning-writeback-live`，真实 `build/bin/MilkSU.app` 使用隔离 App data 创建 `CVE-2023-46604` tracking workspace、写入 1 条 learning、从 projection 恢复读取，报告 `build/test-results/vuln-learning-writeback-live.json` 的四个 gate 全为 true，且不包含原始学习正文或 key 形态内容 | 不再把“CVE 研究结论只能留在 localStorage”或“完整研究结果写回没有打包 App 证据”作为未完成；WebView UI 链路由 `DONE-CVE-08` 关闭，资产验证 UI 写回由 `DONE-CVE-09` 关闭，真实 Vulhub 用户目录 UI 绑定由 `DONE-CVE-10` 关闭 |
| DONE-CVE-08 | 打包 App WebView UI 能完成 CVE 导入 Coding 结论并投影正式档案 | `app_vuln_learning_writeback_webview_smoke.go` 新增只在 `MILKSU_VULN_LEARNING_WRITEBACK_WEBVIEW_SMOKE_RESULT` 存在时开启的 smoke-only Wails 钩子；`vulnerabilityLearningWritebackWebViewSmoke.ts` 在真实打包 App WebView 中打开 CVE 工作台、选中目标 CVE、点击“导入 Coding 结论”、填写用户确认摘要、点击“导入到笔记”，再调用 `GetVulnJob` 读回 projection；2026-08-05 `MILKSU_VULN_LEARNING_WRITEBACK_WEBVIEW_LIVE_SMOKE=1 npm run test:vuln-learning-writeback-webview-live` 通过，报告 `build/test-results/vuln-learning-writeback-webview-live.json` 显示 `CVE-2023-46604`、正式研究档案 `job_56077706-4a0d-4af7-aa2b-67ce585c02c6`、1 条 learning，且所有 UI/projection gate 为 true，不含原始学习正文或 key 形态内容 | 不再把“导入 Coding 结论缺打包 App UI 链路证据”作为未完成；如用户需要，后续可做人工/Computer Use 点击复核，但那是扩样，不再阻塞 CVE 写回链路 |
| DONE-CVE-09 | 打包 App WebView UI 能新增 CVE 资产验证并投影正式档案 | 新增 `asset_verification.recorded` 领域 fact、`RecordVulnAssetVerification` App/Wails facade、`VulnPage.vue` 资产表单正式写回、`vulnerabilityAssetVerificationWebViewSmoke.ts` 和 `scripts/test-packaged-vuln-asset-verification-webview-live.mjs`；2026-08-05 `MILKSU_VULN_ASSET_VERIFICATION_WEBVIEW_LIVE_SMOKE=1 npm run test:vuln-asset-verification-webview-live` 使用真实 `build/bin/MilkSU.app` 打开 CVE 工作台、选中 `CVE-2023-46604`、通过 UI 新增 `packaged-smoke-activemq` 资产，再调用 `GetVulnJob` 读回正式研究档案 `job_6f83d5ce-5791-4b78-87a2-c529b33b51ae` 和 1 条 asset verification；报告 `build/test-results/vuln-asset-verification-webview-live.json` 所有 UI/projection gate 为 true，且不包含原始资产地址或 key 形态内容 | 不再把“CVE 资产验证只能留在 localStorage/没有打包 App UI 链路证据”作为未完成；这只是用户确认资产进入影响检查，不等于真实漏洞复现、外部资产可被利用或批量扫描能力 |
| DONE-CVE-10 | 打包 App WebView UI 能绑定真实 Vulhub 用户目录 | 新增 `app_vuln_practice_directory_webview_smoke.go`、`vulnerabilityPracticeDirectoryWebViewSmoke.ts` 和 `scripts/test-packaged-vuln-practice-directory-webview-live.mjs`；2026-08-05 `MILKSU_VULN_PRACTICE_DIRECTORY_WEBVIEW_LIVE_SMOKE=1 npm run test:vuln-practice-directory-webview-live` 会 sparse checkout `github.com/vulhub/vulhub` 当前 commit `aeaf65793f147f29bd50841ef77f4e9cad07ecc7` 的 `activemq/CVE-2023-46604`，校验 `docker-compose.yml`，用真实 `build/bin/MilkSU.app` 打开 CVE 工作台、选中 `CVE-2023-46604`、通过 UI 确认练习计划、调用目录选择链路绑定该目录，并验证 UI 显示本地目录和“启动本地练习”；报告 `build/test-results/vuln-practice-directory-webview-live.json` 所有 gate 为 true，且不记录完整本地 Vulhub 路径或 key 形态内容 | 不再把“真实 Vulhub 用户目录 UI 绑定”作为未完成；这只证明目录绑定和启动入口可见，不启动 Docker、exploit、漏洞触发输入或外部目标流量，也不证明资产存在漏洞 |
| DONE-CU-01 | 打包 Computer Use proxy 能在外部 App 精确窗口 Scope 内执行可见点击 | `scripts/test-packaged-computer-use-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_COMPUTER_USE_LIVE_SMOKE=1` 运行真实 `build/bin/MilkSU.app` sidecar node、`computer-use-proxy.cjs` 与 `cua-driver`，对 `com.apple.calculator` PID/window 完成 observe → click 1 → observe，报告保存到 `build/test-results/computer-use-live.json`，前后截图保存到 `build/test-results/computer-use-live-reset.png` 与 `build/test-results/computer-use-live-after.png`；after 截图显示 Calculator 显示区为 `1` | 不再把“packaged Computer Use proxy 是否能操作外部 App”当作未完成；UI 选择/启动和重启清理由 `DONE-CU-03`/`DONE-CU-04` 关闭，工程视觉链路由 `DONE-CU-05` 关闭，剩余只看真实外部 Provider 模型质量与原生 UI 人工验收 |
| DONE-CU-02 | 打包 MilkSU App facade 能启动并停止外部 App 精确会话 | `app_coding_computer_use_smoke.go` 新增只在 `MILKSU_COMPUTER_USE_APP_SMOKE_RESULT` 存在时运行的 App 内部 smoke；`scripts/test-packaged-computer-use-app-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_COMPUTER_USE_APP_LIVE_SMOKE=1 npm run test:computer-use-app-live` 会自动打开 Calculator，并让真实 `build/bin/MilkSU.app` 进程完成 `ListCodingComputerUseTargets` → `StartCodingComputerUse` → `GetCodingComputerUseStatus` → descriptor socket 检查 → `StopCodingComputerUse`，报告保存到 `build/test-results/computer-use-app-live.json`，目标为 `com.apple.calculator` PID `85601` window `61879` | 不再把“Wails/App facade 是否能把用户选定外部 App 变成可用 Computer Use descriptor”作为未完成；UI 选择/启动和重启清理由 `DONE-CU-03`/`DONE-CU-04` 关闭，工程视觉链路由 `DONE-CU-05` 关闭，剩余只看真实外部 Provider 模型质量与原生 UI 人工验收 |
| DONE-CU-03 | 真实 MilkSU UI 内选择 Calculator 并启动/停止可见会话 | 2026-08-05 重新打包当前 HEAD 的 `/Users/milksu/code/milksu/build/bin/MilkSU.app`，用隔离 `MILKSU_APPDATA_DIR=/tmp/milksu-cu-ui-live/app-data` 启动真实 App；通过 Computer Use 操作 MilkSU UI：CTF → Coding → 右栏 `浏览器与 App` → 目标下拉选择 `计算器 · 计算器` → 点击 `启动可见会话`；状态刷新后显示 `接入状态 已接入当前任务`、锁定 `com.apple.calculator · PID 85601 · Window 61879`，并可见 `停止可见会话`；随后从 UI 点击停止，状态回到 `可启动`。证据保存到 `build/test-results/computer-use-ui-live.json` 和 `build/test-results/computer-use-ui-live-after-start.png` | 不再把“MilkSU UI 内是否能选择/启动外部 App 可见会话”作为未完成；重启清理由 `DONE-CU-04` 关闭，工程视觉链路由 `DONE-CU-05` 关闭，剩余只看真实外部 Provider 模型质量与原生 UI 人工验收 |
| DONE-CU-04 | 打包 MilkSU 重启后不会幽灵恢复旧 Computer Use 会话 | 2026-08-05 用隔离 `MILKSU_APPDATA_DIR=/tmp/milksu-cu-restart-live.tiPNBu/app-data` 启动真实 `build/bin/MilkSU.app`，在 Coding → 浏览器与 App 中选择 `计算器 · 计算器` 并启动可见会话，before 文本包含 `接入状态 已接入当前任务`、`com.apple.calculator · PID 85601 · Window 61879` 和 `停止可见会话`；随后通过 Computer Use 直接操作外部 Calculator 完成 `2+3=5`，重启同一 App data 后回到 Coding → 浏览器与 App，任务 `计算器 可见会话` 仍保留，但详情页显示 `接入状态 可启动`，旧 Calculator PID/window 不再作为已接入 Scope；报告保存到 `build/test-results/computer-use-restart-live.json`，截图保存到 `build/test-results/computer-use-restart-live-calculator-operation.png` 和 `build/test-results/computer-use-restart-live-after-restart.png` | 不再把“Computer Use UI 启动后的 App 重启清理/权限一致性”作为未完成；工程视觉链路由 `DONE-CU-05` 关闭；剩余缺口是真实外部 Provider 模型质量与原生 UI 人工验收 |
| DONE-CU-05 | 打包 Coding Bridge 的纯文本主模型能消费 Computer Use 截图辅助视觉后操作外部 App | `scripts/test-packaged-computer-use-agent-vision-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_COMPUTER_USE_AGENT_VISION_LIVE_SMOKE=1 npm run test:computer-use-agent-vision-live` 使用真实 `build/bin/MilkSU.app` packaged sidecar node、Chat Bridge 和 Cua Driver 自动打开 Calculator，锁定 `com.apple.calculator` 精确 PID/window，主模型第一轮调用 selected `milksu-computer-use` MCP observe 并返回截图，辅助视觉通过当前 Pi `modelRuntime` 与 OpenAI Responses 兼容路径生成截图描述，主模型第二轮在看到 `[MilkSU Computer Use visual evidence]`、视觉描述和 AX/tree 证据后调用 click；测试断言无 approval_requested、前后截图 SHA-256 不同，并保存 `build/test-results/computer-use-agent-vision-live.json`、`computer-use-agent-vision-live-before.png`、`computer-use-agent-vision-live-after.png` | 不再把“工程链路是否能把 Computer Use 截图交给纯文本主模型并回到工具操作”作为未完成；仍不能把它算作真实外部 Provider 模型质量或最终 GUI 自主性，因为本 smoke 使用确定性本地 Provider |
| DONE-CU-06 | Settings 页面能重新检测 Computer Use 权限与签名状态 | `SettingsPage.vue` 在通用设置中新增 Computer Use 权限/签名诊断区，`重新检测` 只调用 `get_coding_computer_use_status`，不会打开系统授权；当 ad-hoc/不稳定签名导致探针与系统设置可能不一致时，UI 禁用重复授权并提示先稳定签名或重启后复检；`SettingsPage.test.ts` 覆盖只读刷新不调用 `RequestCodingComputerUsePermissions` 和不稳定签名不鼓励反复授权；`app_settings_computer_use_webview_smoke.go`、`settingsComputerUseWebViewSmoke.ts` 与 `scripts/test-packaged-settings-computer-use-webview-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_SETTINGS_COMPUTER_USE_WEBVIEW_LIVE_SMOKE=1 npm run test:settings-computer-use-webview-live` 用真实 `build/bin/MilkSU.app` 打开 Settings，点击重新检测并读回 `accessibility=true / screenRecording=true`，报告 `build/test-results/settings-computer-use-webview-live.json` 所有 gate 为 true，且不含 key 形态内容 | 不再把“MilkSU 设置面板重新检测”作为 COD-16 缺口；Developer ID 签名、公证与升级仍属于发行门禁，不回填到 COD-16 |
| DONE-CTF-01 | CTF 工作台跨模块恢复浏览器烟测 | 2026-08-05 在 `http://127.0.0.1:1432/` 的真实渲染界面点击 CTF P382/gift_F12 工作台 → CVE → Coding → CTF，返回后仍显示 NSSCTF 列表、P382 详情、CTF rail 高亮，Console 无 error/warn | 不再把“CTF 一级菜单切回后空白/跳错模块”作为待修；打包 persistence 由 `DONE-CTF-02` 关闭 |
| DONE-CTF-02 | 打包 App CTF job / Agent workspace / checkpoint / replay 跨重启恢复 smoke | `app_ctf_recovery_smoke.go` 新增只在 `MILKSU_CTF_RECOVERY_SMOKE_RESULT` 存在时运行的 App 内部 smoke；`scripts/test-packaged-ctf-recovery-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_CTF_RECOVERY_LIVE_SMOKE=1 npm run test:ctf-recovery-live` 使用真实 `/Users/milksu/code/milksu/build/bin/MilkSU.app` 和隔离 HOME/App data 连续启动 create/verify 两阶段，恢复同一 `job_6db30598-3369-477a-8eb6-aab87530d005`、`ctf_be5903fcda3d6c38` workspace、running checkpoint、3 条 replay events、Agent progress 和同一 material digest；报告保存到 `build/test-results/ctf-recovery-live.json`、`build/test-results/ctf-recovery-live-create.json`、`build/test-results/ctf-recovery-live-verify.json` | 不再把“打包 App 中 CTF 运行中 job/workspace/checkpoint/replay 能否跨重启恢复”作为未完成；这不是 Solver 模型、平台 Judge、漏洞触发输入或六赛道真实成绩 |
...
确认切到产品闭环冲刺。该冲刺已于 2026-08-05 随 PR #1 合并到 `main`。后续从本台账和
当前目标中选择有界批次，不按旧候选批次恢复，也不因单个非阻塞问题切回深度优先。
...
| 层级 | 台账范围 | 当前处理 |
| --- | --- | --- |
| P0 · 自主工作 | `COD-01`–`COD-05`、`COD-07`、`COD-08`、`COD-10`、`COD-15`、`COD-16`、`COD-17`–`COD-25`、`COD-31`、`RUN-03` | 先补尚未形成最小纵切的必要能力；长任务恢复的 deterministic 与 WebView UI 路径已关闭，后续重点是构建测试、恢复、Computer Use 实用范围和交付，不补完整审批矩阵 |
