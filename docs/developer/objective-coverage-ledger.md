...
用户第一轮评估已经写入 `current-objectives.md`。产品开发和持久目标继续暂停；当前只讨论
首批范围，不按旧候选批次自动开始修复。
...
| 层级 | 台账范围 | 当前处理 |
| --- | --- | --- |
| P0 · 自主工作 | `COD-01`–`COD-05`、`COD-07`、`COD-08`、`COD-10`、`COD-17`–`COD-25`、`RUN-03`、`RUN-08` | 从中再选首批；重点是长任务、构建测试、恢复和交付，不补完整审批矩阵 |
| P1 · 成熟能力 | `COD-12`–`COD-16`、`COD-26`–`COD-30` | 优先复用成熟组件；每项至少一个真实任务 |
...
此前的“候选广度批次 A”已被本映射取代，不再具有候选或执行意义。选择首批后仍需为每项
先写清：
