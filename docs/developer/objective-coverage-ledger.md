...
使用真实 MilkSU 源码隔离 clone，让 Coding runtime 新增真实脚本/测试、运行 `node --test`，
再由真实打包 App Git facade commit/push 到隔离 bare remote，将 `COD-25` 从 50% 调到 55%；
这仍不是真实外部 Provider、Vue+Go 联动、私有远端 PR Gate 或 Hosted PR 发布。
后续只使用同一张表比较变化。
...
| 55% | 1 | 已有真实源码隔离 clone 自举预演，但仍缺最终 Gate |
...
| DONE-COD-05 | Coding runtime 能在真实 MilkSU 源码隔离 clone 中完成 edit/test，并由打包 App Git facade 交付 | `scripts/test-packaged-milksu-source-self-bootstrap-live.mjs` 新增 gated live smoke；2026-08-05 显式 `MILKSU_MILKSU_SOURCE_SELF_BOOTSTRAP_LIVE_SMOKE=1 npm run test:milksu-source-self-bootstrap-live` 从当前 MilkSU 仓库克隆真实源码到隔离目录，Coding runtime 使用打包 sidecar node 读取 `package.json` 和 `app/src/App.vue`，写入 `scripts/lib/milksu-source-self-bootstrap-smoke.mjs` 与 `tests/scripts/milksu-source-self-bootstrap-smoke.test.mjs`，运行 `node --test tests/scripts/milksu-source-self-bootstrap-smoke.test.mjs`，再用真实 `build/bin/MilkSU.app` 的 Git facade stage/commit/push 到隔离 bare remote；报告 `build/test-results/milksu-source-self-bootstrap-live.json` 显示 `toolCalls=[read,read,write,write,bash]`、`providerPlanConsumed=true`、`toolFailures=0`、`remoteHead=c373d47378a5`、工作区 clean，且实际比较运行前后主工作区 `git status` 确认 active worktree 未被 smoke 污染 | 不再把 COD-25 说成只有 toy fixture；这仍不是当前主工作区自改、真实外部 Provider、Vue+Go 联动、私有远端 PR 或 Hosted PR 发布 |
