...
| OBS-14 | `ChatPage.vue` 的 Computer Use UI 已抽出并测试 PID/window pair 选择与启动参数，但还没有完整组件级测试覆盖右侧栏交互、Select 组件和 Wails invoke 时序 | 后续若建立 ChatPage 测试基架或做打包 App 外部窗口验收时补，不在当前批次深挖 |
