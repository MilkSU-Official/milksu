...
| OBS-19 | CTF、CVE、Coding 顶栏模块标题已统一到 `WorkspaceTopBarTitle`，但页面内部二级标题、统计数字、卡片按钮和下拉菜单仍可能存在视觉系统不一致 | 2026-08-04 用户要求三个一级菜单同位置标题必须同一组件和同一大小；本批次新增 `WorkspaceTopBarTitle.vue`，由 `WorkspaceTopBar.vue` 统一调用；Browser preview `http://127.0.0.1:4185/` 依次验证 CTF/CVE/Coding 的 `[data-workspace-topbar-title]` 均为 `H1 + workspace-topbar__title`，渲染字号 `14px`、行高 `20px`、字重 `450`，顶栏 action 区 `14px`，console 无 warn/error | 一级模块导航条不会再因为页面各自手写标题而漂移；内部组件视觉债仍会影响整体精致度 | 顶栏已修；内部视觉系统按后续 UI sweep 广度清单处理，不在当前批次深挖 |
