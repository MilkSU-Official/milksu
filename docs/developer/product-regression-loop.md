...
| 入口 | 问的问题 | 怎么跑 |
| --- | --- | --- |
| 本页 · 产品回归 | 改完功能后，上手、主页、看板娘、意图识别、领域工作区、桌面执行面、资料和设置其余项还成不成立 | `npm run test:product-loop` |
...
跑这条命令的 agent 就是监督者。没有第二套提示词，也没有另一个模型在打分。进程里原有的窗口监视只负责关掉多开的 MilkSU，不读用例结果。

监督者看着同一条 stdout，一项落地再看下一项。程序写下的 `PASS` / `FAIL` / `SKIP` 保持原样，不要改成自己的印象分。`FAIL` 当时就写一行复查：失败原因，以及后面的项还会不会因此全失败或一直超时。

后面的项都会白跑时，停掉这个进程：

- 网络、模型服务、Key、额度、账户发不出。
- 代码问题会让后面继续失败或超时。停掉之后先修，再从头跑。不要带着已知的坏结果把套件跑完。

只影响当前项的失败让套件继续。复查写在当次对话里，放在该项程序结果旁边。不要把 Key 写进复查、回执或提交。
...
`上手 → 主页 Coding → 看板娘 → 意图识别 → 领域工作区 → 桌面执行面 → 账户与更新 → 设置其余项`

不附着已经在首页的日常窗口。开测前和每条用例前关掉日常安装包和残留 Electron，只留这一扇测试窗。登录回调若先落到别的进程，只把回调交回发起登录的那个程序，然后这一扇退出，不另开窗口，也不弹「另一扇窗口」。跑的过程中每隔几秒数一次窗口，多出来的关掉，并让当前项失败。不要越开越多。Key 打进设置密码框，不注入 sidecar。中转站能发之后，主页发送缺来源记 FAIL。
...
| 账户模型 | 用账户额度让模型列目录、写一份带 `PRODUCT-LOOP-NOTES` 的 `NOTES.md` 再读回来。没连上或没额度：SKIP，不记 PASS，也不把来源标就绪。Key 被拒或发不出：FAIL。 |
| 设置中转站 | 进设置 → 模型，把端点、名字、模型和 Key 打进密码框并保存。协议跟填写的端点走，思考字段跟模型家族走，不把中转站写成没有思考的 OpenAI Chat Completions。没有 Key、也没有已经存过的中转站，算失败。Key 被拒不是「记预期」通过。先试官方 DeepSeek，再试 TokenFlux。 |
| 中转站模型 | 用刚配好的中转站再让模型写一份带 `PRODUCT-LOOP-NOTES` 的 `NOTES.md`。写不出来就不能往下测，算失败。 |
| 暂不登录 | 关掉再开一次，必须再看见登录页。点「暂不登录」进首页，刚才的中转站还在。 |
| 未登录走主模型 | 个人中转站能发、账户已退出。发一句短问。折叠来源是主模型，记录写明主模型。这时候不能还有账户发下的钥匙。 |
| 登录后发下意图识别钥匙 | 再登录之后，设置回执有钥匙、不是本次会话手填、回执里没有钥匙明文。没有就失败，不往设置里写钥匙。 |

### 主页 Coding（35）

离开设置，还在刚才那扇窗里，并且已经回到已登录账户。Coding、看板娘、意图识别和后面的模块都用账户模型和账户发下的意图识别钥匙。个人中转站只覆盖「暂不登录」那一段，不再把后面的用例改到个人 Key。

| 测试项 | 测什么 |
| --- | --- |
| Pi 写文件 | 用 Pi 列目录、写出 `NOTES.md`、再读回来。文件里要有 `PRODUCT-LOOP-NOTES`，并且真的调用了文件工具。只聊天不算。 |
| Pi 后台子代理 | 必须调用一次 `subagent`，让 `worker` 沿用当前会话模型写出 `SUBAGENT.txt`。只聊天、父会话自己写文件、或子代理报模型找不到，都不算。 |
| Pi 跑命令 | 用 shell 写出 `DATE.txt`，里面有 `hello-product-loop`。 |
...
| DSH 写文件 | 换成 DSH 再写一遍带 `PRODUCT-LOOP-NOTES` 的 `NOTES.md`。 |
| DSH 跑命令 | DSH 写出带 `hello-product-loop` 的 `DATE.txt`。 |
...
| 换看板娘模型再发 | 换成另一台模型后再发出一句。账户目录和个人中转站都算。只有一台：SKIP，不记 PASS。 |
| 看板娘模型设置 | 设置 → 看板娘有模型选择。 |
| 跨会话调度设置 | 设置里有跨会话调度。 |
...
默认全量里，排在看板娘后面。`npm run test:product-loop -- --gui --suite all` 会跑。单独跑：`npm run test:product-loop -- --gui --suite intent`。

产品名是意图识别，英文 Intent。折叠记录和工具调用同一层。设置里没有钥匙框，也写不进钥匙。已登录时钥匙只来自账户下发。判定看折叠记录的来源字段，不看回复像不像。

| 测试项 | 测什么 |
| --- | --- |
| 账户发下意图识别钥匙 | 已登录，设置回执有钥匙，不是本次会话手填，回执里没有钥匙明文，设置页也没有钥匙框。缺任何一条就失败。不往设置里写钥匙。 |
| 设置里没有钥匙 | 设置 → 看板娘没有意图识别或 Jev 的钥匙输入。 |
| 设置里写不进钥匙 | 经设置保存写一把钥匙留不下来，也不把账户已经发下的钥匙清掉。 |
| 闲聊直接回 | 短问她直接答。折叠记录是闲聊。不派会话，也不事后补一句。 |
| 深入思考仍在这一轮 | 她自己答完，不派会话。记录是深入思考。推理档不变。 |
| 长任务派出去 | 派到一条对话。过程中不逐条播报工具。记录是长任务。 |
| 追问不另派 | 长任务还在跑时问进度或刚才改了什么。她汇报这条会话，不另派一条。 |
| 做完她通知 | 她派成功的会话回合结束后，她通知并给建议。没有一条假装是用户说的话。 |
| 两条通知都在 | 她派成功的两条会话差不多同时做完。两条标题都在同一次通知里，后一条不盖掉前一条。 |
| 待批她通知 | 弹出批准时她说一句在等你点。她不替你点。点了同意之后那条会话继续，这次等待不算做完。 |
| 真报错她通知 | 凭据撤回、模型拒绝、余额或权限不够、写会话失败时她说一句。限流和上下文溢出不先开口。 |
| 工具失败后还盯着 | 她派出去的命令先返回失败，会话自己改完并继续。她不把这次失败当成回合结束。真正做完时仍通知。 |
| 卡住才问说不说 | 进度还在走时她不说话。进度停了才出现一次说不说的记录。 |
| 记忆留或丢 | 稳定称呼留下，一次性碎片不留下。记录里有这次记忆判断。 |
| 没派的会话不说 | 不是她派成功的会话结束时，她不补一句。 |
| 开口时不串话 | 她正要通知时，用户发出一条新问题。两段按顺序各走一轮，通知正文不拼进用户那句话。 |
| 没接上标明主模型 | 上手已经在未登录窗口看过，就再核登录后钥匙还在。单独跑这一项时先退出登录，短问的来源必须是主模型，然后再登录，钥匙要回来。不在设置里摘钥匙。 |
...
| Lab 自定义任务 | 自定义任务分段在。 |
| Lab 空任务 | 没有任务时点回去，题目包要出现。已经有任务则这条失败，不把有任务记成空态通过。 |
| Lab 创建任务 | 打得开创建表单。 |
...
| Computer Use 观察计算器 | 必须有权限，而且本机开着计算器。观察并写下 `SURFACE.md`。缺权限或没开计算器都算失败，不改走浏览器。独立窗口没有打包驱动记 SKIP，不记 PASS。 |
...
| 模型 | 默认运行时、默认模型、忙碌时发送都在。上手配过的中转站还在，或者账户已登录。 |
...
- 本机先填 `docs/developer/product-loop.local.env`（模板是旁边的 `.example.env`）。协调器读入公开字段；密钥只留在脚本内存，到设置密码框再填，不注入 sidecar。回执只写变量名。个人中转站只在退出登录的那一段使用。再登录成功并且账户发下意图识别钥匙之后，看板娘和主页留在账户来源。文件循环没写出文件，不改走个人 Key。账户没回来时，也不把后面的用例改到个人 Key。
