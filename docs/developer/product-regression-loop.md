...
专题，不在默认全量里。`npm run test:product-loop -- --gui --suite intent`。
...
- 本机先填 `docs/developer/product-loop.local.env`（模板是旁边的 `.example.env`）。协调器读入公开字段；密钥只留在脚本内存，到设置密码框再填，不注入 sidecar。回执只写变量名。个人中转站只在退出登录的那一段使用。账户文件循环通过并且再登录成功之后，看板娘和主页留在账户来源，意图识别用账户发下的钥匙。账户没回来时，不把后面的用例改到个人 Key。
