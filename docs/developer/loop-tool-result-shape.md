...
>
> 缺什么：过长结果仍把原文的头塞回模型。溢出已落盘，但下一跳 `edit` 缺少稳定的路径/行号/（可选）哈希。
...
- [ ] 短输出字节级等于工具原文（允许末尾换行规范化，不允许改写成摘要）。
- [ ] 过长输出：模型可见长度不超过现行 Pi 边界；notice 指出 overflow 路径；`read`+offset 能读到被切掉的部分。
- [ ] 相对 `main` 基线：下一轮 input 中该 tool_result 的 token、同一文件重复 `read` 次数下降，且编辑成功率不下降。
- [ ] 无新用户可见页面。截断提示若改措辞，中英 `t()` 成对。
- [ ] 不把完整 HTTP/命令/文件 dump 写进 `content`。
