...
> 文档状态：**Target / Sidecar+Composer implemented; automatic argument-gate not done**
>
> 选定范围：运行中用户输入如何进入 **当前** Pi 回合。这是人在同一条 Loop 里插话，不是第二模型，也不是新开一条用户回合。
>
> 缺什么：自动参数闸（3a）未做。Electron 真会话手测「打字中途插话」未跑。
> 已实现：纯文本流式时先 `steer` 再 `agent.abort()`；bash / edit / write 等工具在跑时只排队。
> Composer 区分「已并入本回合」与「当前工具调用结束后应用」。
...
**本切片未做该自动参数闸（3a）。** 流式 `edit`/`write`/`bash` 不在 `steerSession` 里做规则检查；见 `sidecar/pi/bridge-steering.js` 与 `bridge-steering.test.js`。
...
- [x] 运行中发送不创建第二条并列的 user 回合来代表「下一条对话」。
- [ ] assistant 纯文本流式阶段：从发送到模型上下文包含该句的延迟有上限（秒级 abort，而不是等完整 assistant 消息）。Electron 真会话未手测。
- [x] 工具执行阶段：引导最晚在该工具结束后的下一次模型调用出现。
- [x] Composer：中英 `t()` 成对。「已排队」改为能区分「等待当前工具结束」与「已并入本回合」。空控件保持空白，不写教练式说明。
- [x] 附件在 steering 路径仍拒绝（现状），或在本 PR 写明为何改变并补测。
- [x] 自动参数闸若未做：PR 标明未做，不把 3a 写成已完成。
- [x] 无侧边聊天、无顾问模型。
...
**Summary：** 旧基线（只 `session.steer`、不 abort stream）已被本切片取代。MilkSU 在 assistant 纯文本流式且没有未完成工具时先把引导写入当前 turn 的 steer 队列，再 `agent.abort()` 当前 stream，让 Pi 的下一跳带上引导；bash / edit / write 在跑时仍只 `steer`。实现覆盖见 `sidecar/pi/bridge-steering.test.js`。Pi 0.84.1 源码里 `steer()` 仍写「工具批次结束后再交下一跳」，那是上游注释，不是 MilkSU 现在的行为。
