...
README 仍是产品介绍，不是项目日志，也不是设计文档。可以收紧或重排一小节，不要改掉现在的写法：
居中标题、徽章、一张主图、短的能力介绍、一小格截图。

同一轮还要核对这三块，对着刚发出的安装包，不要对着更晚的 `main`：

- **鸣谢**：看上一版 tag 之后合进 `main` 的贡献。新贡献的同学补进行和头像。已经在表里的同学，把他们这一版新合进来的 PR 补进同一行。不要编名字，也不要把这张表写成提交清单。
- **开源组件**：这一版新带上的组件补进表（它在 MilkSU 里做什么、许可）。已经卸掉、不再进安装包的删掉。细节留在 NOTICE，表本身保持短。
- **截图和开篇介绍**：换成这一版界面里用户真能看到的样子。介绍只写现在能做的事，拿掉已经不在产品里的说法。

| 文件 | 必须改写的事实 |
| --- | --- |
| `README.md` | 下载徽章、下载链接和“当前状态”里的最新回执版本。这是唯一宣称 Latest 的地方。同一轮按上面三条更新鸣谢、开源组件、截图和开篇介绍。 |
