...
> 文档状态：**Implemented / 已随 26.917.3 发行**
>
...
4. Composer（React）：斜杠两项中英 `t()` 成对；最后一条可丢掉的用户消息显示「丢掉这段」；用量环面板提供「整理上下文」和「接到新会话」。`running` 时 rewind 可点（先 abort），handoff 与 `/新任务` 一样禁用；compaction 进行中两项都禁用。任意更早的用户消息仍走「编辑并从这里重发」。
