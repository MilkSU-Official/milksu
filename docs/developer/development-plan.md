...
| Coding 日常工作流 | 已完成窄真实闭环 | 输入框上方仅保留 Plan/Go、权限和模型；项目、能力、架构图、变更、终端和浏览器集中在右侧。原生 App 已连续完成理解、修改、测试、修复和总结；本地远端已验 stage、commit、push；右侧终端同时提供按 Conversation 隔离的交互式项目 PTY，以及可显示 PID/端口/有界日志并停止的后台任务；右侧浏览器页已通过固定 Playwright MCP 完成隔离 Chrome 的启动、页面读取、填写、点击和结果回读；TypeScript/Vue LSP 已固定随包并在原生 fixture 返回 `TS2322 @ 1:14` | 仍缺 `gopls`、`lsp_fix` 审批、逐块/行级审阅、托管平台 PR、跨应用重启终端恢复、Computer Use、多 Agent 和更大项目样本 |
