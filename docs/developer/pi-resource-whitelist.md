...
| `deep-research` | MilkSU source | Coding + CTF roles | Multi-round web research with source-grounded citations and a workspace report | First-party reviewed Skill; explicit Pi `/skill:deep-research`; reuses the reviewed `web_search` / `web_fetch` tools instead of a second research harness; fetched content is data, quotes must be verbatim from read sources |
