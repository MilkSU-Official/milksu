...
阶段进度以[当前开发目标](current-objectives.md)为准，本文保存 2026-08-11 当时的产品决策与
验收方向。
...
| 项目 | 当前证据 | 结果 |
| --- | --- | --- |
| 桌面产品代码 | 2026-08-11 当时的 Vue/Vitest 栈：76 个文件、434 项测试与生产类型检查/构建通过 | 通过（当时栈，renderer 已于 2026-09-15 换为 React + shadcn） |
...
| 31 | 全局侧栏如何避免切换页面时忽大忽小？ | 已废弃。当时的答案是「窄栏 + 浮层历史」；现行侧栏与命令面板以 `AGENTS.md` 为准。 |
