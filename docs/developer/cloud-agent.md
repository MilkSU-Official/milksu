...
第一包：**Cloudflare Sandbox**。镜像自带与桌面同钉的 **Pi 与 DSH**；按会话只起其中一个。实例档位从 `basic` 起评，不够再升，不默认 `standard-4`。
...
- 对话列复用 Coding 输入栏契约（胶囊、模型芯片、运行时 Pi/DSH、审批、用量）。云上可选模型来自账户目录；运行时选择与桌面新对话同一语义。  
