...
| 责任 | 所有者 |
| --- | --- |
| 模型 Session、压缩、Tool Loop | 云里的 **Pi**（与桌面同钉版本；不另造 harness） |
...
1. Admin 给用户发余额 → 桌面登录 → 建云会话 → 一回合流式（含至少一次沙箱内 shell）→ `turn.settled` 扣费 → 余额可见。  
2. 同一账户在第二台客户端（先桌面双开或预览页，再 iOS）打开同一会话看到连续事件。  
3. 余额为 0 时拒绝新回合，文案明确。

## 成功怎么算

- 真账户：发额度 → 云回合成功 → ledger 有模型 + 沙箱两条（或等价拆分）流水。  
- 真机桌面：云环境发一句，流式 + 工具过程 + 结算后余额变化。  
- 真机 iOS：登录 → 发一句 → 流式；与桌面同一 `sessionId`。  
- 逻辑测试：无 Bearer 拒；余额不足拒；Key 不上客户端；并发沙箱上限生效。
...
3. Cloud API + CF Sandbox + Pi 镜像；桌面云环境冒烟。  
4. 计费结算挂 `turn.settled`；余额不足路径。  
5. iOS 一阶段登录 + 云对话。  
6. 自助充值、Android、更强持久化 / 多会话策略：后置。

不要先做手机再把协议绑死在临时 HTTP；不要先做 #131 扫码冒充云 Agent。

## 关键节点

1. 准入即本页；计费重新进入产品链。  
2. 沙箱选定 CF；退 E2B 必须有装不下 Pi 的证据。  
3. 服务端持 Key；客户端只持账户 session。  
4. 桌面云一回合 + 扣费。  
5. iOS 云一回合。  
6. 与 #131 远程控制分发行线，互不阻塞。
