...
>
> 夹具完成：`sidecar/pi/bridge-edit-anchor.js` 提供哈希绑定锚点，Coding/CTF 共用同一
> `wrapEditToolDefinition`。过期哈希拒绝写入；连续解析/锚点失败后同一回合降回 Pi
> `replace`。真模型对照未跑，计费链路待用户授权。不得把夹具写成真实 Coding 完成。
...
- [x] 未改代码的基线数字写进本 PR 或 Evidence；没有基线不算完成。
- [x] 假 Provider / 单测覆盖：过期哈希拒绝、坏补丁降级、短文件精确替换仍走 Pi `edit`。
- [ ] 真模型对照若已跑：失败 `edit` 次数或「成功写入前工具次数」相对基线有数量级或成倍下降；几个百分点不够。
- [x] 真模型对照若未跑：PR 标明「夹具完成、计费链路待用户授权」，不得把夹具写成真实 Coding 完成。
- [x] 10/25/150 重复刹车回归仍通过。
- [x] 不把 Provider Key 写入工具参数、日志或补丁器测试夹具。
- [x] 无新用户可见页面。工具行最多多一句失败原因，中英 `t()` 成对。
...
`@oh-my-pi/hashline` 审阅结论：许可证是 MIT、npm 可单独钉版本，但运行时依赖 `@oh-my-pi/pi-natives` / `@oh-my-pi/pi-utils`，默认 `NodeFilesystem` 走 Bun，标签是 SnapshotStore 的 4-hex token。未引入该包，也未引入 Oh My Pi 整包。

自有层：`sidecar/pi/bridge-edit-anchor.js`。补丁绑 SHA-256（允许前缀）。格式是 `contentHash` + `edits[].anchor`，或紧凑块 `[path#hash]` / `PUT start.=end:`。过期哈希拒绝且不降级。`contentHash` 同时认磁盘原文与 LF/BOM 归一后的 SHA-256，这样 Windows CRLF 和工具回流 16 位前缀不会被误判过期；带行号的截断 preview 哈希仍不是文件哈希。连续 2 次解析/锚点失败且同一次调用带了 `oldText`/`newText` 时，降回 Pi `replace`。Coding 与 CTF 共用包装器。10/25/150 刹车未改。
