...
> 能跑，并让 CVE 作为一级菜单具备学习/追踪工作台骨架；Lab 暂不做自建平台或本地靶场。
...
| CVE / Vuln | 具备“学习与追踪”工作台骨架：CVE 情报、关注、研究任务、资产命中、学习路径、安全边界和后续 Agent 可接手任务 | 批量打靶、红队 Agent、自动 PoC 复现、披露流程 |
...
| CVE | 追踪列表、学习笔记、影响检查 | CVE record 持久化后，用户需要在多个 CVE 间切换 |
...
CVE 作为一级菜单进入当前几个小时冲刺的最小实现目标；Lab 不进入当前实现。
...
CVE 模块当前和长期都先定位为学习与追踪，不是批量打靶或自动攻击。当前冲刺至少需要让用户
能看出它可以追踪一个 CVE，并把后续工作交给 Coding Agent 继续：
...
几个小时内不拆“学习/审计/修复/复现”四套复杂模式；先做一条可见骨架，后续再由 Coding
Agent 迭代持久化记录、真实 Feed、授权仓库影响检查和复盘沉淀。

## 暂停但保留入口的范围

- Lab 不进入当前冲刺实现目标；
- CVE 进入当前冲刺的学习/追踪骨架目标，但不提供红队 Agent 或批量攻击能力；
- 不运行未经审核的漏洞触发输入、附件、服务或 exploit；
- 不把 UI 架子描述成真实安全研究能力；
- 不把开源项目写进候选表等价为已经成为依赖；
- 真实接入前仍需许可证、供应链、架构、离线/网络、资源和 Judge 审查。
...
5. CVE 保留学习/追踪骨架并记录后续 Agent 可接手任务；Lab 后置记录外部靶场辅助/进度追踪计划；
