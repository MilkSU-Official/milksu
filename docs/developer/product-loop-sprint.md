...
- Session Index：首轮原生闭环已通过；CVE Note、Coding 输入、CTF 复盘草稿的用户确认路径
  ...
  隔离 clone，让 Coding runtime 新增真实 Node 脚本/测试、Go 源码/测试和前端 TS/Vitest
  文件，运行 `node --test`、`go test ./internal/sourcebootstrap` 与
  `npm --prefix app run test -- --run src/lib/sourceSelfBootstrapSmoke.test.ts`，再由真实
  打包 App Git facade commit/push 到隔离 bare remote。
...
- Computer Use：packaged proxy → Cua Driver → 外部 Calculator 精确窗口的 live smoke 已通过；
  ...
  前后截图变化；Settings 页面已接入 Computer Use 权限/签名诊断和只读重新检测，packaged
  Settings WebView live smoke 证明真实打包 App 可打开设置页、点击重新检测并读回当前授权状态；
  下一步只补真实外部 Provider 与原生 UI 人工验收；
