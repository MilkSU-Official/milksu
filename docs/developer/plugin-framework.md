...
框架针对用户主动信任的发布者提供供应链和故障隔离。它不宣称是同一操作系统用户下的强敌对多租户沙箱。
...
看板娘角色皮肤不是这六个表面。`app.pet` / `ui.pet` 按 [看板娘皮肤设计合同](companion-skin.md) 交帧；启用后设置 → 看板娘可以选这套皮肤。文件夹导入是同一条换装入口，不要求先做成插件。
