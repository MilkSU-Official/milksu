...
| 事实 | 当前状态 |
| --- | --- |
| 正式发行基线 | `v26.818.1 / b92fcded25e59805463af4ff718d73ed8776e3fb`（2026-08-18）。这是最近一次带三端产物与 SHA-256 的内测包；标签固定在该 source commit，后续 workflow/文档提交不移动标签。 |
| 已发行线 | `26.817.1` 建立账户 TokenFlux / 双来源路由与 Pi 网页查证；`26.817.2` 补齐 Linux 试用 DEB 与 Windows 启动；`26.817.3` 修 Windows 账户授权恢复与打包 Sidecar 路径；`26.818.1` 发出账户可调用目录、Windows 目录 Reveal、上下文圆环计量与今日三端安装包。 |
| 开发版本线 | 仓库版本仍为 `26.818.1`。`origin/main` 已晚于发行源 `b92fcde`，含 Coding 多标签浏览器、`milksu_workspace`、对话级批准、85% 自动压缩、CTF 本地目录视图和未打包 Windows 登录回调。 |
| 三端内测发行 | GitHub prerelease `v26.818.1` 已提供签名并公证的 macOS ARM64 DMG、未签名 Windows x64 EXE 和 Linux x64 试用 DEB，以及 `SHA256SUMS-26.818.1.txt`；R2/Admin current pointer 未发布。 |
| Linux | `26.818.1` DEB 已包含当时的 Pi Runtime 收敛并通过原生 Ubuntu 包结构、Node/Pi、Go Runtime 与 Xvfb Electron 启动；仍不含 Secret Service、本地 OCR 或 Computer Use。 |
...
| 最近发行回执 | macOS 本机签名公证 DMG；Windows `32095638823`、Linux `32095642259` 均成功。文件名、大小和 SHA-256 记录在当前开发目标。 |
...
| 文档 | 状态 | 负责什么 | 不负责什么 |
| --- | --- | --- | --- |
| [当前开发目标](current-objectives.md) | Current / Canonical | 当前阶段、正式发行基线、开发版本线、已发行/未发版事实、下一完成线和后置项 | 不保存完整聊天、微提交或旧验收过程 |
...
- 下载页与对外状态以 `26.818.1` 回执为准；晚于 `b92fcde` 的 `main` 只描述当前仓库，不能改写该发行页。
