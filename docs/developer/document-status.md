...
| 事实 | 当前状态 |
| --- | --- |
| 正式发行基线 | `v26.823.1 / efeda10af4f1e2cf55c4a8db1761cdbb486055a2`（2026-08-23 今日首发）。这是当前 GitHub Latest Release；标签固定在该 source commit，后续 workflow/文档提交不移动标签。 |
...
| 开发版本线 | 仓库版本为 `26.823.2`。正式发行基线仍是 `v26.823.1 / efeda10`；版本号提升和本地构建不移动该 tag，也不构成新的正式发行。 |
| 许可证 | 主项目为 `AGPL-3.0-only`（`LICENSE` / `NOTICE`）。第三方仍保留原许可。Obelisk 作为计划嵌入的 AGPL 记忆组件与此兼容；尚未 vendored。 |
| 三端正式发行 | GitHub Latest Release `v26.823.1` 已提供签名并公证的 macOS ARM64 DMG、未签名 Windows x64 EXE 和 Linux x64 试用 DEB，以及 `SHA256SUMS-26.823.1.txt`；R2/Admin current pointer 未发布。 |
