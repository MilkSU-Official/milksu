...
| Linux | `26.825.1` DEB 已包含当前 Pi Runtime 收敛并通过原生 Ubuntu 包结构、Node/Pi、Go Runtime 与 Xvfb Electron 启动；仍不含 Secret Service、本地 OCR 或 Computer Use。四发行版安装面与 ISSUE #19 边界见 [Linux 四发行版支持合同](linux-platform-support.md)；该合同是 Target，不改变当前发行声明，也不是实施队列。 |
