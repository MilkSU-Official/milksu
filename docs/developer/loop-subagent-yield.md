...
>
> 夹具完成：`bridge-subagent-yield.js` 交出 `status` / cwd 或 worktreeId / `files[]` / `findings[]` / `exitCode`；
> 父侧 `readSubagentYieldField` 零次额外工具可读 `files[0]`。名单在工具活动组，空名单不画说明。
> 未改 `pi-subagent-runner.cjs`。真模型对照未跑。
...
- [x] 假 Provider：父侧 tool_result 含约定字段，测试断言路径取值，不解析散文。
- [x] 只读 vs writer 隔离回归（现有 `bridge-policy` / `pi-subagent-runner` 测试仍过）。
- [x] 父 abort ⇒ 子进程退出（有测试，不只注释）。本切片未改 runner。
- [x] UI：至少一个活状态名单；产品 UI 见 `AGENTS.md`；无第二条对话、无空状态教练文案。
- [x] 不把子 Agent 的 Key、绝对本机家目录或 relay 凭据写进 yield。
- [x] 不把「有按钮」写成「父 Loop 已会用字段」；Loop 完成以夹具取字段为准。真模型对照待授权。
...
环境栏或工具活动组里的子任务名单。产品 UI 见 `AGENTS.md`；不新造一种卡片。
